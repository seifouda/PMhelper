...
class PERTAnalyzer:
    ...
    def analyze(self, activities_data):
        ...
        if self.critical_paths and self.critical_paths[0]:
            self.project_variance = self.calculate_project_variance(self.G, self.critical_paths[0])
    ...
    def get_project_statistics(self):
        ...
        # CRITICAL FIX: Calculate expected duration from critical path, not EF values
        if self.critical_paths and self.critical_paths[0]:
            # Calculate expected duration from critical path expected times
            expected_duration = 0
            critical_path = self.critical_paths[0]
    ...
    def crash_project(self, target_duration, max_iterations=300, max_budget=None):
        ...
        if not ef_values:
            raise ValueError("No activities found with EF values. Check graph structure.")
        ...
        while current_duration > target_duration and iteration < max_iterations:
            ...
            critical_activities = [node for node in crashed_G.nodes() 
                                if crashed_G.nodes[node].get('float', float('inf')) == 0 and node not in ['START', 'END']]
    ...
    def generate_rcps_schedule_for_graph(self, G, resource_limit, priority_rule='minimum_slack'):
        ...
        for node in G.nodes():
            if node not in ['START', 'END']:
                ...
                # Use current durations from the graph (expected_time ceiling values)
