    ...
    def create_gantt_chart(G, critical_activities=None, figsize=(18, 12)):
        ...
        for i, activity in enumerate(activities_data):
            ...
            ax.barh(
                ...
                color=color,
                alpha=0.8,
                label='Early Schedule' if i == 0 else "")
            ...
            if activity['float'] > 0:
                ax.barh(
                    ...
                    color='yellow',
                    alpha=0.6,
                    label='Float' if i == 0 else "")

        # Customize chart
        ax.set_yticks(y_pos)
        ax.set_yticklabels(
            [f"{act['id']}: {act['activity']}" for act in activities_data])
        ax.invert_yaxis()
        ax.set_xlabel('Time')
...
class ScheduleTableVisualizer:
    ...
    @staticmethod
    def create_schedule_table_plot(
        cmp_table,
        rcps_table,
        time_cols,
        critical_ids,
...
def create_comparison_chart(cmp_results, pert_results, figsize=(12, 8)):
    ...
    # Duration comparison
    cmp_duration = max([cmp_results.nodes[node]['EF']
                       for node in cmp_results.nodes()])
    pert_duration = max([pert_results.nodes[node]['EF']
                        for node in pert_results.nodes()])
