...
class SelectionFileHandler:
    ...
    def load(filepath: str | Path) -> SelectionProblem:
        ...
        if filepath.suffix != SelectionFileHandler.EXTENSION:
            raise ValueError(
                f"Invalid file extension. Expected {SelectionFileHandler.EXTENSION}, "
    ...
    @staticmethod
    def export_to_json(problem: SelectionProblem, filepath: str | Path) -> None:
        """
    ...
    def from_csv(filepath: str | Path, method: str) -> SelectionProblem:
        ...
        try:
            ...
            if method in ['ahp', 'linear_scoring']:
                ...
                criterion_cols = [col for col in df.columns if col not in ['ID', 'Name']]
                ...
                for _, row in df.iterrows():
                    scores = {col: row[col] for col in criterion_cols if pd.notna(row[col])}
                ...
                for _, row in df.iterrows():
                    projects.append(Project(
                        ...
                        life=int(row['Life']) if pd.notna(row.get('Life')) else None,
                        annual_om=float(row.get('Annual O&M', 0)),
                        salvage=float(row.get('Salvage', 0))
                    ))
