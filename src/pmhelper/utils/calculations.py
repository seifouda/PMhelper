...
class TimeCalculations:
    ...
    @staticmethod
    def calculate_pert_estimates(optimistic: float,
                                 most_likely: float,
                                 pessimistic: float) -> Tuple[float,
                                                              float]:
        """
    ...
    def calculate_confidence_interval(
            mean: float, std_dev: float, confidence_level: float = 0.95) -> Tuple[float, float]:
        """
...
class CostCalculations:
    ...
    @staticmethod
    def calculate_cost_per_time_saved(
            crash_cost: float,
            time_saved: float) -> float:
        """
    ...
    @staticmethod
    def calculate_crash_efficiency(normal_duration: float,
                                   crashed_duration: float,
                                   total_crash_cost: float) -> Dict[str,
                                                                    float]:
        """
        ...
        time_reduction_percent = (
            time_saved / normal_duration) * 100 if normal_duration > 0 else 0
        cost_per_time_unit = total_crash_cost / \
            time_saved if time_saved > 0 else float('inf')
...
class ResourceCalculations:
    ...
    def calculate_resource_utilization(
            scheduled_resources: List[int], available_resources: int) -> Dict[str, float]:
        """
        ...
        utilizations = [
            res / available_resources for res in scheduled_resources]
    ...
    @staticmethod
    def calculate_resource_leveling_metrics(
            resource_usage: List[int]) -> Dict[str, float]:
        """
        ...
        leveling_index = math.sqrt(variance) / \
            mean_usage if mean_usage > 0 else 0
...
class ProbabilityCalculations:
    ...
    @staticmethod
    def calculate_duration_for_probability(
    ...
    def calculate_risk_metrics(
            expected_duration: float, std_deviation: float) -> Dict[str, float]:
        """
        ...
        """
        coefficient_of_variation = std_deviation / \
            expected_duration if expected_duration > 0 else 0
...
class NetworkMetrics:
    ...
    def calculate_criticality_metrics(G) -> Dict[str, float]:
        ...
        critical_activities = [
            node for node in activities if G.nodes[node].get(
                'float', 1) == 0]
