...
class ProjectService:
    ...
    async def create_project(
        ...
        try:
            ...
            logger.info(
                f"Created project: {db_project.id} - {db_project.name}")
            return db_project
    ...
    async def get_project(
            db: AsyncSession,
            project_id: str) -> Optional[Project]:
        """Get a project by ID."""
        try:
            ...
            if project:
                logger.debug(
                    f"Retrieved project: {project.id} - {project.name}")
            else:
    ...
    async def get_projects(db: AsyncSession,
                           skip: int = 0,
                           limit: int = 50,
                           search: Optional[str] = None) -> tuple[List[Project],
                                                                  int]:
        ...
        try:
            ...
            query = query.offset(skip).limit(
                limit).order_by(Project.created_at.desc())
            ...
            logger.debug(
                f"Retrieved {
                    len(projects)} projects (total: {total_count})")
        ...
        try:
            ...
            if update_data:
                query = update(Project).where(
                    Project.id == project_uuid).values(
                    **update_data)
    ...
    async def get_project_activities(
            db: AsyncSession, project_id: str) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            logger.debug(
                f"Retrieved {
                    len(activities)} activities for project {project_id}")
            return activities

        except Exception as e:
            logger.error(
                f"Error retrieving activities for project {project_id}: {e}")
            raise
    ...
    async def add_analysis_job_to_project(db: AsyncSession,
        ...
        try:
            ...
            if not project:
                logger.error(
                    f"Project not found for job association: {project_id}")
                return False
            ...
            logger.debug(
                f"Analysis job {job_id} associated with project {project_id}")
            return True

        except Exception as e:
            logger.error(
                f"Error associating job {job_id} with project {project_id}: {e}")
            raise

    @staticmethod
    async def get_project_analysis_jobs(
            db: AsyncSession,
            project_id: str) -> List[AnalysisJob]:
        ...
        try:
            ...
            query = select(AnalysisJob).where(
                AnalysisJob.project_id == project_uuid)
            result = await db.execute(query)
            jobs = result.scalars().all()

            logger.debug(
                f"Retrieved {
                    len(jobs)} analysis jobs for project {project_id}")
            return jobs
            ...
            logger.error(
                f"Error retrieving analysis jobs for project {project_id}: {e}")
            raise

    @staticmethod
    def validate_activities_data(
            activities: List[Dict[str, Any]]) -> tuple[bool, List[str]]:
        ...
        for i, activity in enumerate(activities):
            ...
            for field in required_fields:
                if field not in activity:
                    errors.append(
                        f"Activity {i}: Missing required field '{field}'")
            ...
            try:
                ...
                if duration <= 0:
                    errors.append(
                        f"Activity {activity_id}: Duration must be positive")
            except (ValueError, TypeError):
                errors.append(
                    f"Activity {activity_id}: Invalid duration value")
            ...
            if predecessors:
                for pred_id in predecessors:
                    if pred_id == activity_id:
                        errors.append(
                            f"Activity {activity_id}: Cannot be its own predecessor")
        ...
        for activity in activities:
            ...
            for pred_id in predecessors:
                if pred_id not in activity_ids:
                    errors.append(
                        f"Activity {activity_id}: References non-existent predecessor '{pred_id}'")
