...
class ActivityBase(BaseModel):
    ...
    predecessors: List[str] = Field(
        default=[], description="List of predecessor activity IDs")


class CPMActivity(ActivityBase):
    """Activity model for CPM analysis."""
    resource: Optional[str] = Field(
        None, description="Resource assigned to activity")
    cost: Optional[float] = Field(None, description="Activity cost", ge=0)
    crash_duration: Optional[float] = Field(
        None, description="Minimum duration if crashed", ge=0)
    crash_cost: Optional[float] = Field(
        None, description="Cost to crash to minimum duration", ge=0)
    ...
    def crash_duration_valid(cls, v, values):
        if v is not None and 'duration' in values and v > values['duration']:
            raise ValueError(
                'crash_duration must be less than or equal to normal duration')
        return v
...
class PERTActivity(ActivityBase):
    ...
    def pessimistic_valid(cls, v, values):
        if v is not None and 'optimistic_duration' in values and values[
                'optimistic_duration'] is not None:
            if v < values['optimistic_duration']:
                raise ValueError(
                    'pessimistic_duration must be >= optimistic_duration')
        return v
...
class RCPSActivity(ActivityBase):
    ...
    resource_quantity: float = Field(...,
                                     description="Quantity of resource required",
                                     ge=0)
...
class ProjectCreate(BaseModel):
    ...
    activities: List[Dict[str, Any]
                     ] = Field(..., description="List of project activities")
...
class ProjectUpdate(BaseModel):
    ...
    activities: Optional[List[Dict[str, Any]]] = Field(
        None, description="List of project activities")
    metadata: Optional[Dict[str, Any]] = Field(
        None, description="Additional project metadata")
...
class ProjectResponse(BaseModel):
    ...
    data: Dict[str, Any] = Field(...,
                                 description="Project data including activities")
    metadata: Optional[Dict[str, Any]] = Field(
...
class PERTAnalysisRequest(BaseModel):
    ...
    target_duration: Optional[float] = Field(
        None, description="Target project duration for probability analysis", gt=0)
    confidence_level: Optional[float] = Field(
        0.95, description="Confidence level for analysis", ge=0.5, le=0.99)
    options: Optional[Dict[str, Any]] = Field(
        None, description="Additional analysis options")
...
class RCPSAnalysisRequest(BaseModel):
    ...
    resource_limits: Dict[str,
                          float] = Field(...,
                                         description="Resource availability limits")
    project_id: Optional[str] = Field(
        None, description="Optional project ID to associate with analysis")
    options: Optional[Dict[str, Any]] = Field(
        None, description="Additional analysis options")
...
class AnalysisJobResponse(BaseModel):
    ...
    project_id: Optional[str] = Field(
        None, description="Associated project UUID")
    analysis_type: AnalysisType = Field(..., description="Type of analysis")
    status: JobStatus = Field(..., description="Current job status")
    progress: Optional[str] = Field(
        None, description="Job progress percentage")
...
class AnalysisResultResponse(BaseModel):
    ...
    duration_seconds: Optional[float] = Field(
        None, description="Analysis duration in seconds")
    created_at: datetime = Field(..., description="Job creation timestamp")
    completed_at: Optional[datetime] = Field(
        None, description="Job completion timestamp")
...
class JobListResponse(BaseModel):
    ...
    jobs: List[AnalysisJobResponse] = Field(...,
...
class ValidationErrorResponse(BaseModel):
    ...
    errors: List[Dict[str, Any]
                 ] = Field(..., description="Detailed validation errors")
    timestamp: datetime = Field(
        default_factory=datetime.now,
        description="Error timestamp")
