...
class CriterionRequest(BaseModel):
    ...
    name: str = Field(..., description="Criterion name", min_length=1, max_length=100)
    direction: str = Field(..., description="Optimization direction: 'maximize' or 'minimize'")
    weight: Optional[float] = Field(None, description="Criterion weight (0-1)", ge=0, le=1)
...
class AlternativeRequest(BaseModel):
    ...
    name: str = Field(..., description="Alternative name", min_length=1, max_length=100)
...
class ProjectRequest(BaseModel):
    ...
    life: Optional[int] = Field(None, description="Project life in years", gt=0)
    initial_cost: Optional[float] = Field(None, description="Initial investment cost", ge=0)
    annual_om: Optional[float] = Field(None, description="Annual O&M cost", ge=0)
    salvage: Optional[float] = Field(None, description="Salvage value", ge=0)


class AHPRequest(BaseModel):
    """Request model for AHP analysis."""
    criteria: List[str] = Field(..., description="List of criterion names", min_length=2)
...
class AHPResponse(BaseModel):
    ...
    method: str = "AHP"
    consistency_ratio: float = Field(..., description="Consistency ratio (should be < 0.1)")
...
class LinearScoringRequest(BaseModel):
    ...
    criteria: List[CriterionRequest] = Field(..., description="Criteria with weights and directions", min_length=1)
    alternatives: List[AlternativeRequest] = Field(..., description="Alternatives to evaluate", min_length=2)
    sensitivity_analysis: bool = Field(False, description="Perform sensitivity analysis")


class LinearScoringResponse(BaseModel):
    """Response model for Linear Scoring analysis."""
    method: str = "LinearScoring"
    ranked_alternatives: List[Dict[str, Any]] = Field(..., description="Ranked alternatives")
...
class BenefitCostRequest(BaseModel):
    ...
    projects: List[ProjectRequest] = Field(..., description="Projects to analyze", min_length=1)
    marr: float = Field(..., description="Minimum Attractive Rate of Return", gt=0, le=1)
    analysis_type: str = Field("mutually_exclusive", description="Analysis type: 'independent' or 'mutually_exclusive'")
    
    @field_validator('analysis_type')
    @classmethod
    def validate_analysis_type(cls, v):
        if v not in ['independent', 'mutually_exclusive']:
            raise ValueError("analysis_type must be 'independent' or 'mutually_exclusive'")
        return v
...
class BenefitCostResponse(BaseModel):
    ...
    optimal_project: Optional[Dict[str, Any]] = Field(None, description="Optimal project (for mutually exclusive)")
    viable_projects: Optional[List[Dict[str, Any]]] = Field(None, description="Viable projects (for independent)")
    comparisons: Optional[List[Dict[str, Any]]] = Field(None, description="Incremental analysis comparisons")
    analysis_timestamp: datetime = Field(default_factory=datetime.utcnow)


class ConstraintRequest(BaseModel):
    """Constraint for portfolio optimization."""
    type: str = Field(..., description="Constraint type: 'require', 'exclude', 'dependency', 'at_most_one'")
    projects: List[str] = Field(..., description="Projects involved in constraint", min_length=1)
...
class PortfolioRequest(BaseModel):
    ...
    projects: List[ProjectRequest] = Field(..., description="Projects to consider", min_length=1)
    budget: float = Field(..., description="Total budget available", gt=0)
    constraints: Optional[List[ConstraintRequest]] = Field(None, description="Portfolio constraints")
    time_limit: int = Field(60, description="Solver time limit in seconds", gt=0, le=300)
    sensitivity_analysis: bool = Field(False, description="Perform budget sensitivity analysis")
...
class PortfolioResponse(BaseModel):
    ...
    selected_projects: List[str] = Field(..., description="Selected project names")
    total_cost: float = Field(..., description="Total cost of selected projects")
    ...
    shadow_price_budget: Optional[float] = Field(None, description="Marginal value of budget")
    sensitivity_results: Optional[List[Dict[str, Any]]] = Field(None, description="Budget sensitivity results")
    analysis_timestamp: datetime = Field(default_factory=datetime.utcnow)
...
async def analyze_ahp(request: AHPRequest):
    ...
    **Example Request:**
    ...
    """
    try:
        logger.info(f"Starting AHP analysis with {len(request.criteria)} criteria")
        ...
        response_data = {
            ...
            "weights": {name: float(weight) for name, weight in zip(request.criteria, weights)}
        }
        ...
        logger.info(f"AHP analysis completed. CR: {cr:.4f}, Consistent: {cr < 0.1}")
...
@router.post("/linear-scoring", response_model=LinearScoringResponse, status_code=status.HTTP_200_OK)
async def analyze_linear_scoring(request: LinearScoringRequest):
    ...
    **Example Request:**
    ...
    """
    try:
        logger.info(f"Starting Linear Scoring analysis with {len(request.alternatives)} alternatives")
        ...
        if not np.isclose(total_weight, 1.0, atol=0.01):
            raise HTTPException(
                ...
                detail=f"Criterion weights must sum to 1.0 (current sum: {total_weight:.4f})"
            )
        ...
        if request.sensitivity_analysis:
            ...
            sens_results = analyzer.sensitivity_analysis(data_df, weights, weight_ranges)
            response_data["sensitivity_results"] = sens_results
        
        logger.info(f"Linear Scoring analysis completed. Top alternative: {results_df.iloc[0]['name']}")
...
@router.post("/benefit-cost", response_model=BenefitCostResponse, status_code=status.HTTP_200_OK)
async def analyze_benefit_cost(request: BenefitCostRequest):
    ...
    **Example Request:**
    ...
    """
    try:
        logger.info(f"Starting B/C analysis with {len(request.projects)} projects")
        ...
        logger.info(f"B/C analysis completed. Analysis type: {request.analysis_type}")
...
@router.post("/portfolio", response_model=PortfolioResponse, status_code=status.HTTP_200_OK)
async def optimize_portfolio(request: PortfolioRequest):
    ...
    **Example Request:**
    ...
    """
    try:
        logger.info(f"Starting Portfolio optimization with {len(request.projects)} projects")
        ...
        # Perform sensitivity analysis if requested
        if request.sensitivity_analysis and results["status"] in ["Optimal", "Not Solved"]:
        ...
        logger.info(f"Portfolio optimization completed. Status: {results['status']}, Selected: {len(results.get('selected_projects', []))}")
...
async def get_available_methods():
    ...
    methods = [
        ...
        },
        {
            ...
            "requires": ["criteria", "weights", "alternatives"],
        ...
        },
        {
            ...
            "requires": ["projects", "budget"],
            "optional": ["constraints", "sensitivity_analysis"]
        }
    ]
