...
Uses the shared Sugiyama layout engine (barycenter ordering + virtual nodes)
for crossing minimisation and arrow routing that avoids passing through nodes.
...
def draw_network_diagram_on_ax(ax, G: nx.DiGraph, initial=False):
    ...
    Args:
        ...
        initial: If True, uses a deep copy of the original network.
    """
    if initial:
        G = copy.deepcopy(G)
