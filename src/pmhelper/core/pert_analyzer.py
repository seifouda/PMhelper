...
class PERTAnalyzer:
    def build_cpm_schedule_table(self, df_gantt, resource_limit):
        ...
        # The df_gantt should already have expected times in the duration column
        df = df_gantt.copy()
    ...
    def load_activities_from_pert_data(self, activities_data):
        ...
        for row in activities_data:
            ...
            # For CPM calculations, we still need to use integers, but store the precise value too
            ...
            activities.append({
                ...
                'duration': expected_time,  # Use precise value instead of ceiling for more accurate results
    ...
    def build_network(self, activities):
        ...
        if start_nodes:
            G.add_node('START', duration=0, activity='Start', variance=0, 
                      optimistic=0, most_likely=0, pessimistic=0, expected_time=0, expected_time_ceil=0,
                      min_duration=0, crash_cost=0, resource_demand=0, normal_cost=0)
        ...
        if end_nodes:
            G.add_node('END', duration=0, activity='End', variance=0,
                      optimistic=0, most_likely=0, pessimistic=0, expected_time=0, expected_time_ceil=0,
                      min_duration=0, crash_cost=0, resource_demand=0, normal_cost=0)
    ...
    def analyze(self, activities_data):
        ...
        try:
            ...
            # Phase 2: Calculate scheduling values (integer-based) first to get correct critical path
            scheduling_results = self.calculate_scheduling_values(activities)
            
            # Phase 1: Calculate statistical values using the scheduling critical path
            statistical_results = self.calculate_statistical_values(activities, scheduling_results['critical_paths'])
    ...
    def calculate_statistical_values(self, activities, critical_paths_from_scheduling):
        """Phase 1: Calculate precise statistical values for PERT analysis using scheduling critical path"""
        # Calculate PERT statistics with precise values using critical path from scheduling
        if activities and critical_paths_from_scheduling and critical_paths_from_scheduling[0]:
            ...
            # This ensures variance is calculated on the actual project critical path
            critical_path = critical_paths_from_scheduling[0]
            project_variance = self.calculate_project_variance(statistical_network, critical_path)
    ...
    def calculate_scheduling_values(self, activities):
        ...
        for activity in activities:
            ...
            scheduled_activity['duration'] = math.ceil(activity['expected_time'])
            scheduled_activity['expected'] = math.ceil(activity['expected_time'])  # For display
        ...
        critical_paths, critical_activities = self.network_builder.identify_critical_path(network)
    ...
    def calculate_completion_probability(self, target_duration):
        ...
        if self.critical_paths and self.critical_paths[0]:
            ...
            expected_duration = max([self.G.nodes[node]['EF'] for node in self.G.nodes()])
