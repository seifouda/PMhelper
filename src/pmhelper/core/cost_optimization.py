...
class IndirectCostModel:
    ...
    def __init__(self, cost_categories: Dict[str, float]):
        ...
        logger.info(
            f"Initialized IndirectCostModel with daily rate: ${
                self.daily_rate:.2f}")
...
class TimeCostOptimizer:
    ...
    def generate_curve(self) -> pd.DataFrame:
        ...
        if not hasattr(self.cpm, 'G') or self.cpm.G is None:
            raise ValueError(
                "CPM analysis must be performed before optimization")
        ...
        while iteration < max_iterations:
            ...
            indirect_cost = self.indirect_model.calculate_cost(
                current_duration)
            curve_points.append({
                'duration': current_duration,
                'direct_cost': current_direct_cost,
    ...
    def _find_crashable_activities(
            self, already_crashed: List[str]) -> List[Dict]:
        """
        ...
        for activity_id in self.cpm.critical_activities:
            ...
            if duration > min_duration:
                ...
                crash_cost_slope = (
                    crash_cost - normal_cost) / days_crashable if days_crashable > 0 else float('inf')
    ...
    def find_optimal_duration(self) -> dict:
        ...
        savings_pct = (
            savings /
            normal_point['total_cost'] *
            100) if normal_point['total_cost'] > 0 else 0
        ...
        logger.info(
            f"Optimal duration: {
                result['optimal_duration']} days (savings: ${
                savings:.2f})")
        return result
...
def integrate_cost_optimization_with_cpm(cpm_analyzer):
    ...
    def optimize_cost(self) -> dict:
        ...
        if not hasattr(self, 'indirect_model'):
            raise ValueError(
                "Must attach indirect costs first using attach_indirect_costs()")
    ...
    import types
    cpm_analyzer.attach_indirect_costs = types.MethodType(
        attach_indirect_costs, cpm_analyzer)
    cpm_analyzer.optimize_cost = types.MethodType(optimize_cost, cpm_analyzer)
    cpm_analyzer.get_optimization_curve = types.MethodType(
        get_optimization_curve, cpm_analyzer)
