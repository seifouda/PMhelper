...
class Solution:
    ...
    objectives: Dict[str, float]  # Objective values (duration, cost, npv, etc.)
...
class MultiObjectiveOptimizer:
    ...
    def __init__(self, objective_functions: Dict[str, Callable]):
        ...
        logger.info(f"Initialized MultiObjectiveOptimizer with objectives: {list(objective_functions.keys())}")

    def evaluate_solution(self, schedule: Dict[str, int], metadata: Dict = None) -> Solution:
        """
        ...
        return Solution(schedule=schedule, objectives=objectives, metadata=metadata or {})
    ...
    def compute_pareto_frontier(self, minimize_objectives: List[str] = None) -> List[Solution]:
        """
        ...
        for i, solution in enumerate(self.solutions):
            ...
            for j, other in enumerate(self.solutions):
                if i != j and self.dominates(other, solution, minimize_objectives):
                    is_dominated = True
                    break
        ...
        logger.info(f"Pareto frontier contains {len(pareto_frontier)} of {len(self.solutions)} solutions")
    ...
    def find_tradeoffs(self, obj1: str, obj2: str) -> pd.DataFrame:
        ...
        for i in range(len(sorted_frontier) - 1):
            ...
            delta_obj1 = sol_next.objectives[obj1] - sol_current.objectives[obj1]
            delta_obj2 = sol_next.objectives[obj2] - sol_current.objectives[obj2]
...
class ScheduleGenerator:
    ...
    @staticmethod
    def generate_all_feasible(activities: List, max_combinations: int = 10000) -> List[Dict[str, int]]:
        """
        ...
        non_critical = [act for act in activities if hasattr(act, 'float') and act.float > 0]
        critical = [act for act in activities if not hasattr(act, 'float') or act.float == 0]
        ...
        if total_combinations > max_combinations:
            logger.warning(f"Would generate {total_combinations} schedules, sampling {max_combinations}")
            return ScheduleGenerator.generate_sampled(activities, max_combinations)
    ...
    @staticmethod
    def generate_sampled(activities: List, num_samples: int = 1000) -> List[Dict[str, int]]:
        """
    ...
    @staticmethod
    def generate_heuristic(activities: List, strategies: List[str] = None) -> List[Dict[str, int]]:
        """
