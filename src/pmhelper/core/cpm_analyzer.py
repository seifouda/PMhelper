...
class CPMAnalyzer:
            ...
            if pred_data:
                if isinstance(pred_data, list):
                    ...
                    predecessors = [p.strip()
                                    for p in pred_data if p and str(p).strip()]
                elif isinstance(pred_data, str) and pred_data.strip():
                    # Comma-separated string - split it
                    predecessors = [p.strip()
                                    for p in pred_data.split(',') if p.strip()]
                else:
                    # Convert to string and split (fallback for other types)
                    predecessors = [p.strip() for p in str(
                        pred_data).split(',') if p.strip()]
    ...
    def analyze(self, activities_data):
        ...
        self.critical_paths, self.critical_activities = self.network_builder.identify_critical_path(
            self.G)
        self.G = self.network_builder.calculate_additional_metrics(
            self.G, activities_data)
        self.critical_paths, self.critical_activities = self.network_builder.identify_critical_path(
            self.G)
    ...
    def crash_project(
        ...
        while current_duration > target_duration and iteration < max_iterations:
            ...
            for node in crashed_G.nodes():
                if node not in ['START', 'END']:
                    ...
                    if ef <= current_time:
                        ...
                        print(
                            f"DEBUG: Activity {node} completed at time {ef} (current_time: {current_time})")
            ...
            # Find activities that can be crashed (on critical path, not
            # completed, have crash potential)
