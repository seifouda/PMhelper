...
class DelayRiskAnalyzer:
    ...
    def calculate_expected_delay(self, contract_time: float) -> float:
        ...
        expected_completion = self.expected_duration + self.std_dev * phi / (1 - Phi)
    ...
    def calculate_risk_cost(
        ...
        return {
            ...
            'z_score': (contract_time - self.expected_duration) / self.std_dev if self.std_dev > 0 else 0
        }
...
class VarianceReductionAnalyzer:
    ...
    def analyze_strategies(
        ...
        baseline = baseline_analyzer.calculate_risk_cost(contract_time, penalty_rate)
        ...
        best_name = max(strategies.keys(), key=lambda k: strategies[k]['net_benefit'])
        best_strategy = strategies[best_name]
        best_strategy['name'] = best_name
        
        # Generate summary
        summary = self._generate_strategy_summary(baseline, strategies, best_name)
    ...
    def _analyze_time_reduction(
        ...
        for reduction in np.linspace(0, max_time_reduction, 50):
            ...
            new_risk = new_analyzer.calculate_risk_cost(contract_time, penalty_rate)
            ...
            baseline_risk = baseline_analyzer.calculate_risk_cost(contract_time, penalty_rate)
    ...
    def _analyze_variance_reduction(
        ...
        for reduction in np.linspace(0, max_variance_reduction, 50):
            new_variance = max(self.variance - reduction, 0.01)  # Keep small positive variance
...
"""
        ...
        for name, strategy in strategies.items():
            ...
            summary += f"{indicator} {name}: Net Benefit ${strategy['net_benefit']:,.2f} (ROI: {strategy['roi']:.1f}%)\n"
...
class ActivityRiskPrioritizer:
    ...
    def __init__(self, pert_results: dict):
        ...
        if not self.activities:
            raise ValueError("Activities data is required for risk prioritization")
    ...
    def calculate_risk_scores(self) -> pd.DataFrame:
        ...
        for activity_id, activity in self.activities.items():
            ...
            expected_time = activity.get('expected_time', activity.get('expected_duration', 0))
    ...
    def _generate_activity_recommendation(
        ...
        if cruciality > 0.3:
            recommendations.append("High variance contributor - Reduce uncertainty")
    ...
    def generate_mitigation_plan(
        ...
        medium_priority = df[(df['risk_score'] >= 0.3) & (df['risk_score'] < 0.6)].to_dict('records')
