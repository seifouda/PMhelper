...
class AHPAnalyzer:
    ...
    def set_comparison(self, criterion_i: str, criterion_j: str, value: float):
        ...
        if value < 1 / 9 or value > 9:
            raise ValueError(
                f"Comparison value must be in range [1/9, 9], got {value}")
    ...
    def calculate_geometric_mean(self,
        ...
        for idx, mat in enumerate(matrices):
            if mat.shape != expected_shape:
                raise ValueError(
                    f"Matrix {idx} has shape {
                        mat.shape}, expected {expected_shape}")
    ...
    def __init__(self,
        ...
        for criterion in criteria:
            if criterion not in directions:
                raise ValueError(
                    f"Missing direction for criterion '{criterion}'")
    ...
    def calculate_scores(
        ...
        for criterion in self.criteria:
            if criterion not in alternatives.columns:
                raise ValueError(
                    f"Criterion '{criterion}' not found in alternatives")
    ...
    def sensitivity_analysis(
        ...
        for criterion, (min_weight, max_weight) in weight_ranges.items():
            if criterion not in self.criteria:
                warnings.warn(
                    f"Criterion '{criterion}' not in criteria list, skipping")
                continue
            ...
            for test_weight in np.linspace(min_weight, max_weight, 11):
                ...
                if total_other_weight > 0:
                    for other_criterion in other_criteria:
                        adjusted_weights[other_criterion] -= (difference * (
                            weights[other_criterion] / total_other_weight))
                ...
                try:
                    test_results = self.calculate_scores(
                        alternatives, adjusted_weights)
                    test_ranking = test_results.set_index(
                        'Rank').index.tolist()
...
class BenefitCostAnalyzer:
    ...
    def incremental_analysis(
        ...
        for i in range(1, len(viable_projects)):
            ...
            delta_benefits = challenger['annual_benefits'] - \
                current['annual_benefits']
            delta_om = challenger.get(
                'annual_om', 0.0) - current.get('annual_om', 0.0)

            # Calculate incremental CR (simplified approach)
            delta_cr = challenger['CR'] - current['CR']

            # Incremental B/C ratio
            incremental_bc = delta_benefits / \
                (delta_cr + delta_om) if (delta_cr + delta_om) > 0 else 0
...
class PortfolioOptimizer:
    ...
    def optimize(
        ...
        if status in ['Optimal', 'Not Solved']:
            ...
            # Calculate shadow price of budget (marginal value of additional
            # budget)
            shadow_price = None
            try:
