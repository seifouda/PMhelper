...
class RCPSAnalyzer:
    ...
    def _update_activities_with_rcps_data(self):
        ...
        for activity in self.activities:
            ...
            if activity_id in self.graph.nodes():
                ...
                if 'ES' in node_data:
                    activity['ES'] = node_data['ES']  # Should be actual_start for RCPS
                if 'EF' in node_data:
    ...
    def _build_activities_from_graph(self):
        ...
        for node_id, node_data in self.graph.nodes(data=True):
            activity = {
                ...
                'crash_cost': node_data.get('crash_cost', 100),  # Default crash cost
    ...
    def _create_minimal_network_builder(self):
        ...
        class MinimalNetworkBuilder:
                ...
                for node in nx.topological_sort(G):
                    ...
                    if 'actual_start' in node_data:
                        ...
                        if len(list(G.predecessors(node))) == 0:
                            ...
                            max_ef = max(G.nodes[pred].get('EF', 0) for pred in G.predecessors(node))
            ...
            def backward_pass(self, G):
                ...
                if G.nodes():
                    project_end = max(G.nodes[node].get('EF', 0) for node in G.nodes())
                else:
                    project_end = 0
                ...
                for node in reversed(list(nx.topological_sort(G))):
                    ...
                    if len(list(G.successors(node))) == 0:
                        ...
                        min_ls = min(G.nodes[succ].get('LS', project_end) for succ in G.successors(node))
    ...
    def validate_resource_constraints(self, crashed_activities=None):
        ...
        for node in self.graph.nodes():
            ...
            start_time = int(node_data.get('actual_start', node_data.get('ES', 0)))
