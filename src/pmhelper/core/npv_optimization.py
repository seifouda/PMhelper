...
class NPVOptimizer:
    ...
    def __init__(
            self,
            activities: List[CashFlowActivity],
        ...
        logger.info(
            f"Initialized NPVOptimizer with {
                len(activities)} activities, rate={discount_rate}")
    ...
    def calculate_npv(self, schedule: Dict[str, int]) -> float:
        ...
        for act_id, start_time in schedule.items():
            ...
            if activity.cash_flow != 0:
                pv = activity.cash_flow / \
                    ((1 + self.discount_rate) ** completion_time)
                npv += pv
    ...
    def maximize_npv(self, max_iterations: int = 1000) -> dict:
        ...
        logger.info(
            f"Starting NPV maximization - Original NPV: ${original_npv:,.2f}")
        ...
        if not non_critical:
            ...
            return self._create_result(
                current_schedule, original_npv, best_npv, 0)
    ...
    def _check_precedence(
            self, schedule: Dict[str, int], activity: CashFlowActivity) -> bool:
        """
    ...
    def sensitivity_analysis(self, rate_range: List[float]) -> pd.DataFrame:
        ...
        logger.info(
            f"Sensitivity analysis complete for {
                len(rate_range)} discount rates")
...
def activities_from_cpm_with_cashflows(
        cpm_analyzer, cash_flows: Dict[str, float]) -> List[CashFlowActivity]:
    """
    ...
    logger.info(
        f"Converted {
            len(activities)} activities with cash flows from CPM analysis")
    return activities
