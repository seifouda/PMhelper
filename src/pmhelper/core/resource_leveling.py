...
class ResourceProfile:
    ...
    def __init__(self, schedule: Dict[str, int], activities: List[Activity]):
        ...
        logger.info(
            f"Initialized ResourceProfile with {
...
class MinimumMomentLeveling:
    ...
    def __init__(self, activities: List[Activity],
        ...
        logger.info(
            f"Initialized MinimumMomentLeveling with {
                len(activities)} activities")
    ...
    def level(self, max_iterations: int = 1000,
        ...
        logger.info(
            f"Starting leveling - Original moment: {original_moment:.2f}")
            ...
            for activity in non_critical:
                ...
                for start_time in range(min_start, max_start + 1):
                    ...
                    if self.resource_limit is not None:
                        test_profile = ResourceProfile(
                            test_schedule, activities_list)
                ...
                if best_position != current_schedule[activity.id]:
                    ...
                    logger.debug(
                        f"Iteration {iteration}: Moved {
                            activity.id} to time {best_position}")
                    if record_steps:
                        step_num += 1
                        prof = ResourceProfile(
                            current_schedule, activities_list)
    ...
    def _create_result(self, schedule: Dict[str, int],
        ...
        if original_moment > 0:
            improvement = (
                (original_moment - leveled_moment) / original_moment) * 100
...
class BurgessLeveling:
    ...
    def __init__(self, activities: List[Activity],
        ...
        self.original_schedule = {act.id: act.es for act in activities}
        logger.info(
            f"Initialized BurgessLeveling with {
                len(activities)} activities")
    ...
    def level(self, max_iterations: int = 1000,
        ...
        logger.info(
            f"Starting Burgess leveling - Original cost: {original_cost:.2f}")
        ...
        while improved and iteration < max_iterations:
            ...
            for activity in non_critical:
                ...
                for start_time in range(activity.es, activity.ls + 1):
                    ...
                    test_profile = ResourceProfile(
                        test_schedule, activities_list)
                    if self.resource_limit and not test_profile.is_feasible(
                            self.resource_limit):
                        continue
                ...
                if best_position != current_schedule[activity.id]:
                    ...
                    if record_steps:
                        ...
                        prev_metric = (
                            steps[-1].metric_after if steps else original_cost)
                        steps.append(
                            LevelingStep(
                                step_number=step_num,
                                activity_id=activity.id,
                                ...
                                reason=(
                                    f"Activity {
                                        activity.id} shifted from day {old_pos} " f"to day {best_position} " f"(float={
                                        activity.float}, resource={
                                        activity.resource_demand}) " f"— Burgess cost reduced from {
                                        original_cost:.1f}"),
                                profile=dict(
                                    prof.profile),
                            ))
    ...
    def _create_result(self, schedule: Dict[str, int],
        ...
        if original_cost > 0:
            improvement = (
                (original_cost - leveled_cost) / original_cost) * 100
