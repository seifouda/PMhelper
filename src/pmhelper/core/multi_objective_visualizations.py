...
def plot_pareto_frontier_2d(solutions_df: pd.DataFrame, 
    ...
    if not pareto.empty:
        ...
        ax.scatter(pareto_sorted[obj1], pareto_sorted[obj2],
                  alpha=0.8, s=100, color='red', marker='*',
                  label='Pareto Frontier', zorder=3, edgecolors='darkred', linewidths=1.5)
    ...
    if minimize_obj1 and minimize_obj2:
        ax.annotate('Ideal Direction', xy=(0.05, 0.05), xycoords='axes fraction',
...
def plot_pareto_frontier_3d(solutions_df: pd.DataFrame,
    ...
    # Plot Pareto
    if not pareto.empty:
        ax.scatter(pareto[obj1], pareto[obj2], pareto[obj3],
                  alpha=0.8, s=100, color='red', marker='*',
    ...
    if title:
        ...
        ax.set_title(f'3D Pareto Frontier', fontsize=14, fontweight='bold', pad=20)
...
def plot_npv_sensitivity(sensitivity_df: pd.DataFrame,
    ...
    ax1.plot(sensitivity_df['discount_rate'], sensitivity_df['npv_original'],
            marker='o', linewidth=2, label='Original Schedule (Early Start)', color='blue')
    ax1.plot(sensitivity_df['discount_rate'], sensitivity_df['npv_optimal'],
            marker='s', linewidth=2, label='Optimized Schedule', color='green')
    
    ax1.set_xlabel('Discount Rate', fontsize=11)
    ax1.set_ylabel('Net Present Value ($)', fontsize=11)
    ax1.set_title(title or 'NPV Sensitivity to Discount Rate', fontsize=13, fontweight='bold')
    ...
    ax2.fill_between(sensitivity_df['discount_rate'], 0, sensitivity_df['improvement'],
                     alpha=0.3, color='purple')
    
    ax2.set_xlabel('Discount Rate', fontsize=11)
    ax2.set_ylabel('NPV Improvement ($)', fontsize=11)
    ax2.set_title('Optimization Benefit by Discount Rate', fontsize=13, fontweight='bold')
...
def plot_tradeoff_analysis(tradeoffs_df: pd.DataFrame,
    ...
    ax1.set_title(f'{obj1_label} Along Pareto Frontier', fontsize=12, fontweight='bold')
    ax1.legend()
    ax1.grid(True, alpha=0.3, axis='y')
    
    # Plot 2: Marginal rate of substitution
    if 'marginal_rate' in tradeoffs_df.columns:
        mrs_values = tradeoffs_df['marginal_rate'].replace([np.inf, -np.inf], np.nan)
        ax2.plot(x, mrs_values, marker='o', linewidth=2, color='red', markersize=8)
        ...
        ax2.set_title('Marginal Rate of Substitution', fontsize=12, fontweight='bold')
        ax2.grid(True, alpha=0.3)
...
def plot_objective_comparison(solutions_df: pd.DataFrame,
    ...
    if 'is_pareto' in solutions_df.columns:
        plot_df = solutions_df[solutions_df['is_pareto'] == True].copy()
    else:
    ...
    if normalize:
        for obj in objectives:
            if obj in plot_df.columns:
                ...
                if max_val > min_val:
                    plot_df[f'{obj}_norm'] = (plot_df[obj] - min_val) / (max_val - min_val)
                else:
    ...
    legend_elements = [
        Line2D([0], [0], color='red', linewidth=2, label='Pareto Solutions'),
        Line2D([0], [0], color='gray', linewidth=1, alpha=0.3, label='Dominated Solutions')
    ]
...
def generate_multi_objective_report(optimizer,
    ...
    for idx, row in pareto_df.iterrows():
        ...
        for obj in objectives:
            if obj in row:
                lines.append(f"  {obj.replace('_', ' ').title()}: {row[obj]:.2f}")
        ...
        try:
            if fmt == 'csv':
                ...
                with pd.ExcelWriter(f"{base_path}.xlsx", engine='openpyxl') as writer:
                    pareto_df.to_excel(writer, sheet_name='Pareto Frontier', index=False)
