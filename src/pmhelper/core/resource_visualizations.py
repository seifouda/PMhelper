...
def plot_resource_profile(original_profile: pd.DataFrame,
    ...
    # Original profile
    ax1.bar(original_profile['time'], original_profile['resource_usage'],
            color='lightcoral', edgecolor='darkred', alpha=0.7, label='Resource Usage')
    ...
    # Leveled profile
    ax2.bar(leveled_profile['time'], leveled_profile['resource_usage'],
            color='lightgreen', edgecolor='darkgreen', alpha=0.7, label='Resource Usage')
...
def plot_resource_histogram(original_profile: pd.DataFrame,
    ...
    ax.set_ylabel('Frequency (# of time periods)', fontsize=12, fontweight='bold')
...
def plot_leveling_metrics(result: dict) -> plt.Figure:
    ...
    original_value = result.get('original_moment', result.get('original_cost', 0))
    ...
    if feasible:
        ...
        ax4.text(0.5, 0.35, "Resource limit exceeded",
                ha='center', va='center', fontsize=12, style='italic', color='red')
...
def generate_leveling_report(result: dict, method_name: str = "Resource Leveling") -> str:
    """
...
"""
    ...
    if moved_activities:
        ...
        for act in moved_activities:
            report += f"  {act['id']:8} | {act['original']:14} | {act['leveled']:13} | {act['shift']:+5}\n"
...
def plot_gantt_comparison(original_schedule: Dict[str, int],
    ...
    ax1.set_title('Original Schedule (Early Start)', fontsize=12, fontweight='bold')
    ...
    legend_elements = [
        Patch(facecolor='coral', edgecolor='black', label='Critical (Original)', alpha=0.8),
        Patch(facecolor='lightblue', edgecolor='black', label='Non-Critical (Original)', alpha=0.8),
        Patch(facecolor='darkgreen', edgecolor='black', label='Critical (Leveled)', alpha=0.8),
        Patch(facecolor='lightgreen', edgecolor='black', label='Non-Critical (Leveled)', alpha=0.8)
    ]
...
def export_leveling_results(result: dict, filepath: str, format: str = 'csv') -> None:
    """
