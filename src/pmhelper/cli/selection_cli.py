...
def ahp(criteria, matrix, alternatives, output, verbose):
    ...
    try:
        ...
        if verbose:
            click.echo(f"Loaded {len(criterion_names)} criteria: {', '.join(criterion_names)}")
        ...
        results = {
            ...
            'weights': {name: float(weight) for name, weight in zip(criterion_names, weights)},
            'ranked_alternatives': []
        }
        ...
        if alternatives:
            ...
            for _, row in alt_df.iterrows():
                alt_name = row['name']
                scores = {criterion: row[criterion] for criterion in criterion_names}
...
def score(data, weights, output, sensitivity, verbose):
    ...
    try:
        ...
        if verbose:
            click.echo(f"Loaded {len(data_df)} alternatives with {len(criteria)} criteria")
        ...
        if sensitivity:
            ...
            sens_results = analyzer.sensitivity_analysis(data_df, criterion_weights, weight_ranges)
            ...
            for criterion, variations in sens_results['variations'].items():
                ...
                click.echo(f"\n{criterion}:")
                click.echo(f"  Weight variations causing rank changes: {rank_changes}/{len(variations)}")
            ...
            click.secho(f"✓ Sensitivity results saved to {sens_output}", fg='green')
...
@click.option('--output', '-o', type=click.Path(), required=True,
              help='Output file path for results (JSON)')
@click.option('--analysis-type', '-t', type=click.Choice(['independent', 'mutually_exclusive']),
    ...
    try:
        ...
        if analysis_type == 'mutually_exclusive':
            if results['optimal_project']:
                ...
                click.echo(f"  Annual Benefits: ${opt['annual_benefits']:,.2f}")
                
                if results['comparisons']:
                    click.echo("\nIncremental Analysis:")
                    for comp in results['comparisons']:
                        decision_color = 'green' if comp['incremental_bc'] >= 1.0 else 'red'
                        click.echo(f"  {comp['current']} vs {comp['challenger']}:")
                        click.echo(f"    Incremental B/C: {comp['incremental_bc']:.3f}")
                        click.secho(f"    Decision: {comp['decision']}", fg=decision_color)
            else:
                click.secho("\n✗ No viable projects found (all B/C ratios < 1.0)", fg='red')
        else:
            ...
            if viable:
                click.echo("\nProject Rankings:")
                sorted_projects = sorted(viable, key=lambda x: x['BC_Ratio'], reverse=True)
                for i, proj in enumerate(sorted_projects, 1):
                    click.echo(f"  {i}. {proj['name']}: B/C = {proj['BC_Ratio']:.3f}")
...
@click.option('--verbose', '-v', is_flag=True,
              help='Enable verbose output')
def portfolio(projects, budget, constraints, output, time_limit, sensitivity, verbose):
    """
    ...
    try:
        ...
        if results['status'] in ['Optimal', 'Not Solved']:
            ...
            if selected_projects:
                click.secho(f"\nSelected Projects: {len(selected_projects)}", fg='green', bold=True)
                for proj_name in selected_projects:
                    proj_data = projects_df[projects_df['name'] == proj_name].iloc[0]
                ...
                click.echo(f"Total Annual Benefit: ${results['total_benefit']:,.2f}")
                click.echo(f"Budget Utilization: {results['budget_utilization']:.1f}%")
                click.echo(f"Objective Value: ${results['objective_value']:,.2f}")
                
                if results.get('shadow_price_budget'):
                    click.echo(f"\nMarginal Value of $1 additional budget: ${results['shadow_price_budget']:.2f}")
            else:
                click.secho("\n⚠ No projects selected (budget too low?)", fg='yellow')
        else:
            click.secho(f"\n✗ Optimization failed: {results.get('message', 'Unknown error')}", fg='red')
            ...
            if problem.criteria:
                ...
                if verbose:
                    for criterion in problem.criteria:
                        click.echo(f"  • {criterion.name} ({criterion.direction.value})")
...
@click.option('--method', '-m', type=click.Choice(['ahp', 'linear_scoring', 'benefit_cost', 'portfolio']),
              required=True,
...
def convert(input, output, method, name, description):
    ...
    try:
        ...
        click.echo(f"Items: {len(problem.alternatives or problem.projects or [])}")
