...
class RiskCLI:
    ...
    def load_pert_data(self, filepath: str) -> bool:
        ...
        try:
            ...
            # Normalize column names to lowercase with underscores (PERT analyzer format)
            ...
            if 'id' not in df.columns:
                if 'activity' in df.columns:
                    ...
                    print("Error: CSV must have 'activity', 'task', or 'id' column", file=sys.stderr)
                    return False
    ...
    def cmd_delay(self, args):
        ...
        try:
            # Parse max penalty
            max_penalty = None if args.max_penalty is None else float(args.max_penalty) / 100.0
    ...
    def cmd_contingency(self, args):
        ...
        try:
            # Parse daily cost
            daily_cost = None if args.daily_cost is None else float(args.daily_cost)
    ...
    def cmd_prioritize(self, args):
        ...
        try:
            ...
            if args.format == 'csv':
                ...
                medium_risk = len(df[(df['risk_score'] >= 0.3) & (df['risk_score'] < 0.6)])
                ...
                print(f"\nActivity Details:")
                print(f"{'Activity':<12} {'Risk Score':<12} {'Exp.Time':<12} {'Variance':<12} {'Float':<12}")
                print("-" * 70)
                
                for idx, row in df.iterrows():
                    print(f"{row['activity_id']:<12} {row['risk_score']:<12.3f} "
                          f"{row['expected_time']:<12.2f} {row['variance']:<12.3f} "
                          f"{row['total_float']:<12.2f}")
    ...
    def cmd_report(self, args):
        ...
        try:
            ...
            if args.time_cost and args.variance_cost and args.budget:
                ...
                print("  [Skipped: Requires --time-cost, --variance-cost, and --budget]")
            ...
            if len(high_risk) > 0:
                for idx, row in high_risk.iterrows():
                    print(f"  {row['activity_id']}: Risk={row['risk_score']:.3f}, {row['recommendation']}")
            else:
                print("  None")
...
def create_parser():
    ...
    subparsers = parser.add_subparsers(dest='command', help='Risk analysis commands')
