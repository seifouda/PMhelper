...
from pmhelper.core.multi_objective_visualizations import (
    plot_pareto_frontier_2d, generate_multi_objective_report, export_pareto_solutions
)
...
def optimize_time_cost(args):
    ...
    print(f"Savings: ${result.get('savings', 0):,.2f} ({result.get('savings_pct', 0):.2f}%)")
    print("="*60 + "\n")
    ...
    if args.output:
        ...
        fig = plot_time_cost_curve(curve_data, result, save_path=f"{base_path}_curve.png")
...
def optimize_resources(args):
    ...
    leveler = ResourceLevelingFactory.create(args.method, activities, resource_limit)
    ...
    if args.output:
        ...
        fig = plot_resource_profile(result, save_path=f"{base_path}_profile.png")
...
def optimize_npv(args):
    ...
    if result['moved_activities']:
        ...
        for move in result['moved_activities']:
            print(f"  • {move['id']}: {move['original_start']} → {move['optimal_start']}")
    ...
    if args.sensitivity:
        ...
        if args.output:
            ...
            fig = plot_npv_sensitivity(sensitivity_df, 
                                      save_path=f"{args.output}_sensitivity.png")
            print(f"✓ Saved: {args.output}_sensitivity.png")
            
            sensitivity_df.to_csv(f"{args.output}_sensitivity.csv", index=False)
...
def optimize_pareto(args):
    ...
    def calc_duration(schedule):
        return max(schedule.get(act.id, act.es) + act.duration for act in activities)
    ...
    frontier = optimizer.generate_solutions_grid(schedules, list(objectives.keys()))
    ...
    print(f"Pareto Efficiency: {len(frontier)/len(optimizer.solutions)*100:.1f}%")
    print("\nPareto Solutions:")
...
Examples:
  ...
  pmhelper optimize pareto --input project.csv --objectives duration,cost --samples 100 --output results/pareto
    ...
    subparsers = parser.add_subparsers(dest='command', help='Optimization type')
    
    # Time-Cost Optimization
    tc_parser = subparsers.add_parser('time-cost', help='Time-cost trade-off optimization')
    tc_parser.add_argument('--input', required=True, help='Input project file (CSV/Excel)')
    ...
    # Resource Leveling
    rl_parser = subparsers.add_parser('resources', help='Resource leveling optimization')
    ...
    rl_parser.add_argument('--limit', type=float, help='Resource limit (optional)')
    ...
    npv_parser.add_argument('--input', dest='input_file', required=True,
                           help='Input project file (CSV/Excel)')
    npv_parser.add_argument('--cash-flows', dest='cash_flows', required=True,
                           help='Cash flows JSON file (activity_id: cash_flow)')
    npv_parser.add_argument('--discount-rate', dest='discount_rate', type=float,
                           default=0.10, help='Discount rate (default: 0.10)')
    ...
    # Pareto Frontier
    pareto_parser = subparsers.add_parser('pareto', help='Multi-objective Pareto optimization')
    pareto_parser.add_argument('--input', dest='input_file', required=True,
                              help='Input project file (CSV/Excel)')
    pareto_parser.add_argument('--objectives', required=True,
                              help='Comma-separated objectives (e.g., duration,cost)')
    pareto_parser.add_argument('--samples', type=int, default=100,
