...
def analyze_project(
    ...
    try:
        ...
        print(
            f"Expected Project Duration: {
                stats['expected_duration']:.2f} time units")
        ...
        for duration, description in scenarios:
            ...
            print(
                f"Probability of completing by {
                    duration:.1f} ({description}): {
                    prob:.1%}")
        ...
        for prob in [0.5, 0.8, 0.9, 0.95]:
            ...
            print(
                f"{prob:.0%} probability of completion by: {duration:.2f} time units")
        ...
        if verbose:
            ...
            if stats['critical_activities_variance']:
                ...
                for activity_info in stats['critical_activities_variance']:
                    print(
                        f"  - {activity_info['id']}: Variance = {activity_info['variance']:.3f}")
        ...
        if output_file:
            if format_type.lower() == 'excel':
                FileHandler.save_excel(
                    results_data, output_file, 'PERT_Results')
            else:
        ...
        if verbose:
            ...
            print(
                f"Coefficient of Variation: {
                    risk_metrics['coefficient_of_variation']:.3f}")
            print(
                f"Probability of finishing on expected time: {
                    risk_metrics['probability_on_time']:.1%}")
            print(
                f"Probability of being >10% late: {
                    risk_metrics['probability_10_percent_late']:.1%}")
            print(
                f"Probability of being >20% late: {
                    risk_metrics['probability_20_percent_late']:.1%}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='PERT Analysis Command Line Tool')
    ...
    analyze_parser = subparsers.add_parser(
        'analyze', help='Perform PERT analysis')
    ...
    prob_parser = subparsers.add_parser(
        'probability', help='Perform probability analysis')
    ...
    sample_parser = subparsers.add_parser(
        'sample', help='Generate sample PERT data file')
    ...
    if args.command == 'analyze':
        ...
        try:
            ...
            if args.format.lower() == 'excel':
                FileHandler.save_excel(
                    sample_data, args.output_file, 'Sample_PERT_Data')
            else:
