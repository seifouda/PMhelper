...
class ServerDashboard:
    ...
    def create_widgets(self):
        ...
        # Title
        title_label = ttk.Label(main_frame, text="Server Performance Dashboard", 
        ...
        # Request statistics
        ttk.Label(stats_frame, text="Total Requests:").grid(row=0, column=0, sticky=tk.W, padx=(0, 10))
        self.total_requests_label = ttk.Label(stats_frame, text="0")
        self.total_requests_label.grid(row=0, column=1, sticky=tk.W)
        
        ttk.Label(stats_frame, text="Requests/Min:").grid(row=0, column=2, sticky=tk.W, padx=(20, 10))
        self.requests_per_min_label = ttk.Label(stats_frame, text="0")
        self.requests_per_min_label.grid(row=0, column=3, sticky=tk.W)
        
        ttk.Label(stats_frame, text="Avg Response Time:").grid(row=1, column=0, sticky=tk.W, padx=(0, 10), pady=(5, 0))
        self.avg_response_time_label = ttk.Label(stats_frame, text="0 ms")
        self.avg_response_time_label.grid(row=1, column=1, sticky=tk.W, pady=(5, 0))
        
        ttk.Label(stats_frame, text="Server Uptime:").grid(row=1, column=2, sticky=tk.W, padx=(20, 10), pady=(5, 0))
        ...
        jobs_frame = ttk.LabelFrame(main_frame, text="Analysis Jobs", padding="10")
        jobs_frame.grid(row=2, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(0, 10))
        jobs_frame.columnconfigure(1, weight=1)
        jobs_frame.columnconfigure(3, weight=1)
        
        ttk.Label(jobs_frame, text="Active Jobs:").grid(row=0, column=0, sticky=tk.W, padx=(0, 10))
        self.active_jobs_label = ttk.Label(jobs_frame, text="0")
        self.active_jobs_label.grid(row=0, column=1, sticky=tk.W)
        
        ttk.Label(jobs_frame, text="Completed Jobs:").grid(row=0, column=2, sticky=tk.W, padx=(20, 10))
        self.completed_jobs_label = ttk.Label(jobs_frame, text="0")
        self.completed_jobs_label.grid(row=0, column=3, sticky=tk.W)
        
        ttk.Label(jobs_frame, text="Failed Jobs:").grid(row=1, column=0, sticky=tk.W, padx=(0, 10), pady=(5, 0))
        self.failed_jobs_label = ttk.Label(jobs_frame, text="0")
        self.failed_jobs_label.grid(row=1, column=1, sticky=tk.W, pady=(5, 0))
        
        ttk.Label(jobs_frame, text="Success Rate:").grid(row=1, column=2, sticky=tk.W, padx=(20, 10), pady=(5, 0))
        ...
        active_jobs_frame = ttk.LabelFrame(main_frame, text="Active Analysis Jobs", padding="10")
        active_jobs_frame.grid(row=3, column=0, columnspan=2, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 10))
        ...
        self.jobs_tree = ttk.Treeview(active_jobs_frame, columns=columns, show="headings", height=8)
        ...
        jobs_scrollbar = ttk.Scrollbar(active_jobs_frame, orient=tk.VERTICAL, command=self.jobs_tree.yview)
        ...
        self.performance_text = tk.Text(graph_frame, height=6, width=80, state=tk.DISABLED)
        self.performance_text.grid(row=0, column=0, sticky=(tk.W, tk.E))
        
        # Control buttons
        controls_frame = ttk.Frame(main_frame)
        controls_frame.grid(row=5, column=0, columnspan=2, sticky=tk.W, pady=(10, 0))
        
        ttk.Button(controls_frame, text="Refresh Dashboard", command=self.refresh_dashboard).grid(row=0, column=0, padx=(0, 10))
        ttk.Button(controls_frame, text="Clear Statistics", command=self.clear_statistics).grid(row=0, column=1, padx=(0, 10))
        
        self.auto_refresh_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(controls_frame, text="Auto-refresh (5s)", variable=self.auto_refresh_var).grid(row=0, column=2, padx=(20, 0))
    ...
    def update_performance_display(self):
        ...
        if len(self.request_counts) >= 2:
            recent_avg = sum(count for _, count in list(self.request_counts)[-5:]) / min(5, len(self.request_counts))
            performance_text.append(f"Recent request rate: {recent_avg:.1f} requests/min")
        ...
        if len(self.response_times) >= 2:
            ...
            performance_text.append(f"Response time range: {min_time:.1f} - {max_time:.1f} ms")
        ...
        performance_text.append(f"Debug mode: {'Enabled' if config.DEBUG else 'Disabled'}")
        performance_text.append(f"Network access: {'Enabled' if config.is_network_mode() else 'Localhost only'}")
    ...
    def update_job_progress(self, job_id: str, progress: str, status: str = None):
        """Update job progress."""
