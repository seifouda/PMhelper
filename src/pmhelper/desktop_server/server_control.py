...
class ServerControlPanel:
    ...
    def create_widgets(self):
        ...
        # Server URL
        ttk.Label(status_frame, text="URL:").grid(row=1, column=0, sticky=tk.W, padx=(0, 10), pady=(5, 0))
        self.url_label = ttk.Label(status_frame, text=config.get_server_url(), foreground="blue", cursor="hand2")
        self.url_label.grid(row=1, column=1, sticky=tk.W, pady=(5, 0))
        self.url_label.bind("<Button-1>", self.open_server_url)
        
        # API Documentation URL
        ttk.Label(status_frame, text="API Docs:").grid(row=2, column=0, sticky=tk.W, padx=(0, 10), pady=(5, 0))
        self.docs_label = ttk.Label(status_frame, text=config.get_docs_url(), foreground="blue", cursor="hand2")
        ...
        ttk.Button(log_controls, text="Clear Logs", command=self.clear_logs).grid(row=0, column=0, padx=(0, 10))
        ttk.Button(log_controls, text="Refresh", command=self.refresh_logs).grid(row=0, column=1)
    ...
    def start_server(self):
        ...
        try:
            ...
            messagebox.showerror("Configuration Error", "Invalid port number. Please enter a valid port.")
    ...
    def toggle_network_access(self):
        ...
        if self.network_var.get():
            ...
            self.add_log_message("Network access enabled - server accessible from other computers")
        else:
            config.disable_network_access()
            self.add_log_message("Network access disabled - server accessible from localhost only")
    ...
    def cleanup(self):
        ...
        if hasattr(self, 'gui_handler'):
            logging.getLogger("pmhelper.server").removeHandler(self.gui_handler)
