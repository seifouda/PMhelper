...
class StorageService:
    ...
    def list_charters(self, directory: Optional[str] = None) -> List[Dict[str, any]]:
        """
        ...
        for search_dir in search_dirs:
            ...
            for charter_file in search_dir.rglob("*.json"):
                try:
                    ...
                    project_name = charter.get_field_value('identification', 'project_name') or 'Untitled'
                    ...
                    try:
                        ...
                        template = template_service.load_template(charter.metadata.template_id)
                        if template:
                            completion = charter.get_completion_percentage(template)
                    except Exception as e:
                        print(f"Warning: Could not calculate completion for {charter_file}: {e}")
    ...
    def duplicate_charter(self, filepath: str) -> Optional[str]:
        ...
        try:
            ...
            while new_filepath.exists():
                new_filename = f"{original_path.stem}_copy{counter}{original_path.suffix}"
                new_filepath = original_path.parent / new_filename
                counter += 1
    ...
    def get_export_filepath(self, charter: Charter, extension: str = ".json") -> str:
        """
