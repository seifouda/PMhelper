...
class PDFGenerator:
    ...
    def generate_pdf(
        ...
        try:
            ...
            doc.build(
                story,
                onFirstPage=lambda c,
                d: self._add_page_decorations(
                    c,
                    d,
                    charter,
                    first_page=True),
                onLaterPages=lambda c,
                d: self._add_page_decorations(
                    c,
                    d,
                    charter,
    ...
    def _add_section(self, story: List, charter: Charter, section):
        ...
        for field in section.fields:
            field_value = charter.get_field_value(
                section.section_id, field.field_id)
    ...
    def _add_table_field(self, story: List, field, value: List[List[str]]):
        ...
        header_row = [
            Paragraph(
                str(h),
                self.styles['TableHeader']) for h in headers]
        ...
        for row in data_rows:
            formatted_row = [
                Paragraph(
                    str(cell),
                    self.styles['TableCell']) for cell in row]
    ...
    def _add_page_decorations(
        ...
        if not first_page:
            project_name = charter.get_field_value(
                'identification', 'project_name') or 'Project Charter'
