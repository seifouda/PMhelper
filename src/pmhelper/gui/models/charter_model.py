        ...
        for section in template.sections:
            for field_def in section.fields:
                if field_def.required:
                    total_required += 1
                    value = self.get_field_value(section.section_id, field_def.field_id)
                    if value and self._is_field_filled(value, field_def.field_type):
                        filled_required += 1
    ...
    def validate(self, template) -> List[str]:
        ...
        for section in template.sections:
            for field_def in section.fields:
                if field_def.required:
                    value = self.get_field_value(section.section_id, field_def.field_id)
                    if not value or not self._is_field_filled(value, field_def.field_type):
                        errors.append(
