...
class ProjectCrashing:
    ...
    def run(
        ...
        target_duration = int(
            round(target_duration)) if target_duration is not None else None
    ...
    def _lowest_cost_strategy(
        ...
        for node_id, node_data in G.nodes(data=True):
            if node_id not in ['START', 'END']:
                ...
                print(
                    f"   {status} {node_id}: crash_cost={crash_cost}, normal_cost={normal_cost}, duration={duration}, min_duration={min_duration}")
        ...
        if cost_data_missing > 0:
            ...
            print(
                "   This will cause zero cost calculations in crashing optimization")
        else:
            print(
        ...
        # Use CPM logic from cmp_analyzer.py (forward_pass, backward_pass,
        # calculate_float)
        ...
        print(
            f"[DEBUG CRASHING] Calculated original_duration: {original_duration}")
        ...
        while current_duration > target_duration and iterations < max_iterations:
            ...
            # STEP 1: Calculate potential step normal cost for all activities
            # that would be active in this time step
            step_normal_cost = 0
            ...
            for node_id in G.nodes:
                if node_id not in ['START', 'END']:
                    ...
                    # An activity is active if the current time falls within
                    # its execution window (ES < current_time <= EF)
                    if es < current_time <= ef:
                        # This is treated as a rate
                        step_normal_cost += G.nodes[node_id].get(
                            'normal_cost', 0)
            ...
            completed_activities = {
                n for n in G.nodes if G.nodes[n].get(
                    'EF', 0) <= current_time}
            # print(f"[DEBUG] Completed activities at time {current_time}: {completed_activities}")
            # Build crashable list: critical path, not completed, not at min
            # duration, crash_cost > 0, EF > current_time
            crashable = []

            # 🔍 DEBUG: Investigate crashable activities
            print(f"\n🔨 [ITERATION {iterations}] CRASHABLE ACTIVITY ANALYSIS")
            print(
                f"   Current time: {current_time}, Target duration: {target_duration}")
                ...
                if node not in ['START',
                    ...
                    if float_val == 0:
                        ...
                        if activity_info['can_crash']:
                            crashable.append(
                                (node, crash_cost, dur, min_dur, normal_cost, ef))
                    else:
            ...
            for activity in critical_activities:
                ...
                print(
                    f"      {status} {
                        activity['node']}: crash_cost={
                        activity['crash_cost']}, duration={
                        activity['duration']}, min_duration={
                        activity['min_duration']}")

            print(
                f"   📋 Non-critical activities: {len(non_critical_activities)}")

            print(
                f"   🔨 Final crashable list: {[item[0] for item in crashable]}")
            ...
            for node in G.nodes:
                if (
                    ...
                    normal_cost = data.get('normal_cost', 0)
                    ef = int(round(data.get('EF', 0)))
                    # Only crash if duration > min_duration and crash_cost > 0
                    # and EF strictly greater than current_time
            ...
            if crashable:
                # Prioritize only truly crashable activities by lowest crash
                # cost
                crashable.sort(key=lambda x: x[1])
                # Find the first crashable activity that is eligible (EF
                # strictly greater than current_time)
            ...
            if max_budget is not None:
                projected_total_cost = total_normal_cost_accumulated + \
                    step_normal_cost + total_crash_cost + potential_crash_cost
            ...
            if selected_for_crash:
                ...
                # Calculate the crash amount (we already verified this is valid
                # in the predictive check)
                crash_amount = min(1, dur - min_dur)
                ...
                current_duration = int(
                    round(max(ef_dict.values()))) if ef_dict else 0
                # print(f"[DEBUG] Project duration after CPM recalculation: {current_duration}")

                # Record crash log AFTER CPM recalculation to get correct
                # critical path
                crash_log.append({
                    ...
                    # Duration after this step's CPM recalc
                ...
                # No crashable activities found - check if we should continue
                # or terminate
                print(
                    f"⚠️  [ITERATION {
                        iterations +
                        1}] No more activities can be crashed")
                print(
                    f"   Current duration: {current_duration}, Target: {target_duration}")
                print(
                    "   All critical path activities have reached minimum duration or cannot be crashed")

                # If no activities can be crashed and we haven't reached
                # target, terminate early
                if target_duration is not None and current_duration > target_duration:
                    print(
                        f"   🛑 EARLY TERMINATION: Target duration ({target_duration}) is not achievable")
                ...
                # FIXED: Don't log "Activity None" entries - just advance time
                # and terminate
                print("   ⏭️  No more crashing possible - terminating early")
        ...
        efficiency_metrics = {'cost_per_unit_time': total_crash_cost / (
            original_duration - final_duration) if final_duration < original_duration else 0}
    ...
    def _best_efficiency_strategy(self, *args, **kwargs):
        ...
        raise NotImplementedError(
            "Best efficiency strategy not yet implemented.")
    ...
    def _critical_path_strategy(self, *args, **kwargs):
        ...
        raise NotImplementedError(
            "Critical path priority strategy not yet implemented.")
    ...
    def _resource_aware_strategy(self, *args, **kwargs):
        ...
        raise NotImplementedError(
            "Resource aware strategy not yet implemented.")
...
class RCPSProjectCrashing(ProjectCrashing):
    ...
    def _lowest_cost_strategy(
        ...
        # Recalculating would overwrite actual_start times with theoretical CPM
        # times
        ...
        print(
            f"[DEBUG] RCPS Crashing: Using RCPS duration={original_duration} (not recalculating CPM)")
        ...
        while current_duration > target_duration and iterations < max_iterations:
            ...
            for node in G.nodes:
                if (
                    ...
                    if dur > min_dur and crash_cost > 0 and ef > current_time:
                        # Additional RCPS validation: check resource
                        # constraints
                        if self._validate_crash_feasibility(
                                node, 1, current_time, G):
                            crashable.append(
                                (node, crash_cost, dur, min_dur, normal_cost, ef))
                        else:
            ...
            if selected_for_crash:
                ...
                # No crashable activities found in RCPS - check if we should
                # continue or terminate
                print(
                    f"⚠️  [RCPS ITERATION {
                        iterations +
                        1}] No more activities can be crashed (resource constraints considered)")
                print(
                    f"   Current duration: {current_duration}, Target: {target_duration}")
                print(
                    "   All activities have reached minimum duration or resource constraints prevent crashing")

                # If no activities can be crashed and we haven't reached
                # target, terminate early
                if target_duration is not None and current_duration > target_duration:
                    print(
                        f"   🛑 EARLY TERMINATION: Target duration ({target_duration}) is not achievable with resource constraints")
                    print(f"   Best achievable duration: {current_duration}")
                    termination_reason = 'Target duration not achievable - resource constraints'
                    break

                # Don't log "Activity None" entries - just advance time
                # silently
                print("   ⏭️  Advancing time without logging null activity")
    ...
    def _validate_crash_feasibility(
            self,
            activity,
            crash_amount,
            current_time,
        ...
        try:
            ...
            min_duration = temp_G.nodes[activity].get(
            ...
            network_builder = getattr(
                self.base_analyzer, 'network_builder', None)
            if network_builder:
    ...
    def _enhanced_lowest_cost_strategy(
        ...
        """
        print("[PHASE2] Enhanced RCPS crashing strategy starting...")
        print(
            f"[PHASE2] Target: {target_duration}, Resource Limit: {
                self.resource_limit}")
        ...
        print("[PHASE2] Generating initial RCPS schedule...")
        ...
        if not initial_schedule['schedule_feasible']:
            raise ValueError(
                "Initial RCPS schedule is not feasible with given resource constraints")
        ...
        print(
            f"[PHASE2] Critical activities: {
                initial_schedule['critical_activities']}")
        ...
        while current_duration > target_duration and iterations < max_iterations:
            ...
            print(
                f"[PHASE2] Current time: {current_time}, Duration: {current_duration}, Target: {target_duration}")
            ...
            for activity_id, details in activity_status['status_details'].items(
                ...
                if details['status'] == 'in_progress':
                    step_normal_cost += G.nodes[activity_id].get(
                        'normal_cost', 0)
                    active_activities_this_step.append(activity_id)

            print(
                f"[PHASE2] Step normal cost: ${step_normal_cost}, Active activities: {active_activities_this_step}")

            # STEP 3: Evaluate crash candidates using foundation method
            critical_activities = [
                act for act in G.nodes() if G.nodes[act].get(
                    'float', 0) == 0 and act not in [
                    'START', 'END']]
            ...
            if crash_candidates:
                for i, candidate in enumerate(
                        crash_candidates[:3], 1):  # Show top 3
                    print(
                        f"[PHASE2]   {i}. Activity {
                            candidate['activity_id']}: " +
                        f"Cost=${
                            candidate['crash_cost']}, Reduction={
                            candidate['duration_reduction']}, " +
                        f"Effectiveness=${
                            candidate['cost_effectiveness']:.2f}")
            ...
            if crash_candidates:
                ...
                # Budget checks
                if max_budget is not None:
                    projected_total = total_normal_cost_accumulated + \
                        step_normal_cost + total_crash_cost + potential_crash_cost
                    if projected_total > max_budget:
                        print(
                            f"[PHASE2] Budget limit would be exceeded: {projected_total} > {max_budget}")
                ...
                if max_crash_cost is not None:
                    ...
                    if projected_crash > max_crash_cost:
                        print(
                            f"[PHASE2] Crash cost limit would be exceeded: {projected_crash} > {max_crash_cost}")
                ...
                if max_normal_cost is not None:
                    ...
                    if projected_normal > max_normal_cost:
                        print(
                            f"[PHASE2] Normal cost limit would be exceeded: {projected_normal} > {max_normal_cost}")
            ...
            if selected_candidate:
                ...
                print(
                    f"[PHASE2] Executing crash: Activity {activity_id}, Cost=${crash_cost}, " +
                ...
                print(
                    f"[PHASE2] Crash completed. New project duration: {current_duration}")

            else:
                print(
                    f"⚠️  [PHASE2 ITERATION {iterations}] No viable crash candidates found")
                print(
                    f"   Current duration: {current_duration}, Target: {target_duration}")
                print(
                    "   No activities can be crashed due to resource/schedule constraints")
                ...
                if activity_status['in_progress'] or activity_status['future']:
                    ...
                    if next_times:
                        ...
                        print(
                            f"[PHASE2] Advancing simulation time to: {current_time}")
                    else:
            ...
            if target_duration is not None and current_duration <= target_duration:
                print(
                    f"[PHASE2] Target duration achieved: {current_duration} <= {target_duration}")
                termination_reason = 'Target duration reached'
                break

            # Prevent infinite loops by advancing time if no progress
            if iterations > 1 and current_duration == crash_log[-2].get(
                    'current_project_duration', 0):
                if selected_candidate is None:  # No crash occurred
                    current_time += 1  # Force time progression
                    print(
                        f"[PHASE2] Forcing time progression to: {current_time}")
        ...
        efficiency_metrics = {
            ...
            'duration_improvement_percentage': (
                duration_improvement / original_duration * 100) if original_duration > 0 else 0,
            ...
            'average_cost_per_crash': total_crash_cost / len(
                [
                    log for log in crash_log if log['crash_amount'] > 0]) if any(
                log['crash_amount'] > 0 for log in crash_log) else 0}

        print("\\n[PHASE2] === Crashing Complete ===")
        print(f"[PHASE2] Original duration: {original_duration}")
        print(f"[PHASE2] Final duration: {current_duration}")
        print(
            f"[PHASE2] Duration improvement: {duration_improvement} ({
                efficiency_metrics['duration_improvement_percentage']:.1f}%)")
        print(f"[PHASE2] Total crash cost: ${total_crash_cost:.2f}")
        print(
            f"[PHASE2] Cost per unit time: ${
                efficiency_metrics['cost_per_unit_time']:.2f}")
    ...
    def _intelligent_crash_scorer(
        ...
        print(
            f"[PHASE3] Intelligent scoring for {
        ...
        project_complexity = self._calculate_project_complexity(
            project_context)
        ...
        print(
            f"[PHASE3] Intelligence scoring complete. Top candidate: {
                enhanced_candidates[0]['activity_id']} " +
            f"(Score: {
                enhanced_candidates[0]['intelligence_score']:.3f})")
    ...
    def _calculate_diminishing_returns(
        ...
        base_effectiveness = duration_reduction / \
            (crash_cost + 1)  # Prevent division by zero

        # Adjust for project stage (later crashes are less effective)
        project_progress = project_context.get('completion_percentage', 0)
        # 30% penalty at project end
        stage_penalty = 1.0 - (project_progress * 0.3)
    ...
    def _predict_critical_path_impact(
            self,
            activity_id,
            duration_reduction,
            project_context):
        ...
        critical_path_multiplicity = project_context.get(
            'critical_path_count', {}).get(activity_id, 1)
        multiplicity_bonus = min(0.2, critical_path_multiplicity * 0.05)

        # Boost impact based on activity position in critical path
        activity_position = project_context.get(
            'activity_positions', {}).get(
            activity_id, 0.5)
        # Favor middle positions
        position_factor = 1.0 + (0.5 - abs(activity_position - 0.5)) * 0.4
    ...
    def _assess_resource_optimization(self, activity_id, project_context):
        ...
        activity_resource_req = project_context.get(
            'activity_resources', {}).get(
            activity_id, 1)
        ...
        resource_efficiency = 1.0 - \
            (activity_resource_req / total_resource_limit)
        ...
        optimization_score = resource_efficiency + \
            utilization_bonus - over_utilization_penalty

        return min(1.0, max(0.0, optimization_score))

    def _assess_crash_risks(
            self,
            activity_id,
            duration_reduction,
            project_context):
        ...
        activity_resources = project_context.get(
            'activity_resources', {}).get(
            activity_id, 1)
        resource_utilization = project_context.get('resource_utilization', 0.5)
        resource_risk = max(
            0, (resource_utilization + activity_resources / 10 - 0.8) * 2)

        # Dependency cascade risk
        activity_dependencies = project_context.get(
            'activity_dependencies', {}).get(
            activity_id, 0)
    ...
    def _match_historical_patterns(
        ...
        activity_history = historical_data.get(
            'activity_patterns', {}).get(
            activity_id, [])
        ...
        similar_scenarios = [scenario for scenario in activity_history if abs(
            scenario.get('crash_cost', 0) - crash_cost) <= crash_cost * 0.2]
        ...
        success_count = sum(
            1 for scenario in similar_scenarios if scenario.get(
                'success', False))
    ...
    def _calculate_project_complexity(self, project_context):
        ...
        resource_constraints = project_context.get(
            'resource_constraint_tightness', 0.5)

        # Complexity factors
        # Normalize to 50 activities
        size_complexity = min(1.0, activity_count / 50)
        dependency_complexity = min(
            1.0, dependency_count / (activity_count * 1.5)) if activity_count > 0 else 0
    ...
    def _calculate_scoring_confidence(
        ...
        score_consistency = 1.0 - \
            abs(base_score - intelligence_score) / max(base_score, intelligence_score, 0.1)

        # Confidence based on data availability
        # Assume 10 context items is full
        data_availability = min(1.0, len(project_context) / 10)

        # Confidence based on project characteristics
        complexity = project_context.get('complexity', 0.5)
        # Less confident for complex projects
    ...
    def _pareto_optimal_strategy(
        ...
        """
        print("[PHASE3] Multi-objective Pareto optimization starting...")
        print(
            f"[PHASE3] Objectives: {objectives}, Target duration: {target_duration}")
        ...
        objective_bounds = self._define_objective_bounds(
            G, original_duration, max_budget)

        print(
            f"[PHASE3] Exploring solution space with {
                len(objectives)} objectives...")
        ...
        for strategy in exploration_strategies:
            ...
            strategy_solutions = self._explore_strategy_space(
                ...
                target_duration,
                max_iterations //
                len(exploration_strategies))

            solution_space.extend(strategy_solutions)

        # Identify Pareto-optimal solutions
        print(
            f"[PHASE3] Identifying Pareto-optimal solutions from {len(solution_space)} candidates...")
        pareto_solutions = self._identify_pareto_optimal_solutions(
            solution_space, objectives)

        # Analyze trade-offs between solutions
        trade_off_analysis = self._analyze_pareto_tradeoffs(
            pareto_solutions, objectives)

        # Generate recommendations
        recommendations = self._generate_pareto_recommendations(
            pareto_solutions, trade_off_analysis, objectives)

        computation_time = time_mod.time() - start_time

        print(
            f"[PHASE3] Pareto optimization complete: {
                len(pareto_solutions)} optimal solutions found")
    ...
    def _explore_strategy_space(
        ...
        for iteration in range(max_iterations):
            ...
            varied_weights = self._vary_objective_weights(
                strategy_weights, variation_factor)

            # Generate solution with these weights
            solution = self._generate_weighted_solution(
                G, varied_weights, target_duration, bounds)
    ...
    def _get_strategy_weights(self, strategy, objectives):
        ...
        profile = weight_profiles.get(
            strategy, weight_profiles['balanced_approach'])
        ...
        if total_weight > 0:
            return {
                obj: weight / total_weight for obj,
                weight in selected_weights.items()}
        else:
    ...
    def _vary_objective_weights(self, base_weights, variation_factor):
        ...
        for obj, weight in base_weights.items():
            ...
            variation = random.uniform(-variation_factor,
                                       variation_factor) * weight
    ...
    def _generate_weighted_solution(
            self,
            G,
            objective_weights,
            target_duration,
        ...
        try:
            ...
            while (current_duration >
                   target_duration if target_duration else False) and iteration_count < max_solution_iterations:
                iteration_count += 1

                # Get crash candidates
                critical_activities = [
                    act for act in G_copy.nodes() if G_copy.nodes[act].get(
                        'float', 0) == 0 and act not in [
                        'START', 'END']]
                ...
                objective_scores = []
                for candidate in candidates:
                    score = self._calculate_objective_weighted_score(
                        candidate, objective_weights, project_context)
                ...
                current_duration = best_candidate.get(
                    'new_project_duration', current_duration - 1)
                total_cost += crash_cost
                total_risk += self._assess_crash_risks(
                    activity_id, 1, project_context)
            ...
            objective_values = {
                ...
                # Convert risk to quality-like metric
                'risk': 1.0 - min(1.0, total_risk)
            }
    ...
    def _calculate_objective_weighted_score(
            self, candidate, weights, project_context):
        """Calculate weighted score based on multiple objectives."""
        # Normalize candidate metrics to 0-1 scale
        # Lower cost = higher score
        cost_score = 1.0 / (1.0 + candidate.get('crash_cost', 1))
        time_score = candidate.get(
            'duration_reduction',
            0) / 5.0   # Normalize to typical reduction
        # Higher cost = lower quality
        quality_score = 1.0 - min(0.5, candidate.get('crash_cost', 0) / 100.0)
        risk_score = 1.0 - \
            self._assess_crash_risks(candidate.get('activity_id', ''), 1, project_context)
    ...
    def _build_project_context(self, G):
        ...
        activities = [node for node in G.nodes() if node not in [
            'START', 'END']]
        total_dependencies = len(G.edges())

        # Calculate initial duration using simple CPM
        G_analyzed = self._recalculate_cmp(G)
        original_duration = max([G_analyzed.nodes[node].get(
            'ef', 0) for node in G_analyzed.nodes()])
    ...
    def _define_objective_bounds(self, G, original_duration, max_budget):
        ...
        max_time_reduction = sum([G.nodes[act].get(
            'duration', 1) - G.nodes[act].get('min_duration', 1) for act in activities])
        ...
        for i, solution_a in enumerate(solution_space):
            ...
            for j, solution_b in enumerate(solution_space):
                ...
                for obj in objectives:
                    ...
                    # For cost and risk, lower is better; for time and quality,
                    # higher is better
                    if obj in ['cost', 'risk']:
                ...
                if dominates:
                    # Check if solution_b is actually better in at least one
                    # objective
        ...
        for i, sol_a in enumerate(pareto_solutions):
            for j, sol_b in enumerate(pareto_solutions):
                ...
                for obj in objectives:
                    ...
                    if val_a != 0:
                        trade_off['trade_off_ratio'][obj] = (
                            diff / val_a) * 100
                    else:
                        trade_off['trade_off_ratio'][obj] = float(
                            'inf') if diff > 0 else 0

                # Identify preference scenarios
                if trade_off['objective_differences'].get(
                        'cost', 0) < 0:  # A is cheaper
                    if trade_off['objective_differences'].get(
                            'time', 0) < 0:  # A is slower
                        trade_off['preference_scenarios'].append(
                            'cost_over_speed')
                    else:
                        trade_off['preference_scenarios'].append(
                            'cost_and_speed_optimal')

                trade_offs.append(trade_off)

        # Calculate overall trade-off statistics
        trade_off_stats = self._calculate_tradeoff_statistics(
            trade_offs, objectives)
    ...
    def _calculate_tradeoff_statistics(self, trade_offs, objectives):
        ...
        for obj in objectives:
            differences = [
                t['objective_differences'].get(
                    obj, 0) for t in trade_offs]
            ratios = [
                t['trade_off_ratio'].get(
                    obj, 0) for t in trade_offs if t['trade_off_ratio'].get(
                    obj, 0) != float('inf')]
    ...
    def _generate_pareto_recommendations(
            self,
            pareto_solutions,
            trade_off_analysis,
            objectives):
        ...
        for criterion, solution in recommendations.items():
            ...
            for i, sol in enumerate(pareto_solutions):
                if criterion == 'cost_optimal':
                    ...
                    score = -sol['objective_values'].get('cost', float('inf'))
                elif criterion == 'time_optimal':
                    score = sol['objective_values'].get(
                        'time', 0)  # Higher time reduction is better
                elif criterion == 'low_risk':
                    # Higher risk score is better (lower actual risk)
                    score = sol['objective_values'].get('risk', 0)
                elif criterion == 'high_quality':
                    score = sol['objective_values'].get(
                        'quality', 0)  # Higher quality is better
                elif criterion == 'balanced':
                    # Calculate balanced score as geometric mean of normalized
                    # objectives
                    normalized_scores = []
                    for obj in objectives:
                    ...
                    score = (
                        ...
                        len(normalized_scores)) if normalized_scores else 0
                ...
                if best_score is None or (
                        score is not None and score > best_score):
                    best_score = score
                    best_solution = {
                        'index': i, 'solution': sol, 'score': score}
        ...
        if recommendations['cost_optimal']:
            ...
            summary.append(
                f"💰 Cost-Optimal: ${
                    sol['total_cost']:.2f} crash cost, " f"{
                    sol['original_duration'] -
                    sol['final_duration']} time units saved")
        ...
        if recommendations['time_optimal']:
            ...
            summary.append(
                f"⏱️ Time-Optimal: {
                    sol['original_duration'] -
                    sol['final_duration']} time units saved, " f"${
                    sol['total_cost']:.2f} crash cost")

        if recommendations['balanced']:
            sol = recommendations['balanced']['solution']
            summary.append(
                f"⚖️ Balanced: ${
                    sol['total_cost']:.2f} cost, " f"{
                    sol['original_duration'] -
                    sol['final_duration']} time units, " f"Quality: {
                    sol['quality_score']:.2f}")
    ...
    def evaluate_crash_candidates(
        ...
        current_duration = max([G.nodes[node]['EF']
                               for node in G.nodes() if 'EF' in G.nodes[node]])
        status_data = self.analyze_activity_status(G, current_time)

        for activity in critical_activities:
            eligibility = self._check_crash_eligibility(
                G, activity, current_time, status_data)
            ...
            try:
                ...
                if temp_schedule['schedule_feasible']:
                    ...
                    if duration_reduction > 0:
                        cost_effectiveness = eligibility['crash_cost'] / \
                            duration_reduction
    ...
    def _check_crash_eligibility(self, G, activity, current_time, status_data):
        ...
        eligible = (
            not_finished and has_crash_potential and has_crash_cost and is_critical)
...
def generate_crashing_report(result: CrashingResult) -> str:
    ...
    meaningful_entries = [
        entry for entry in result.crash_log if entry.get('activity') != 'None']

    if not meaningful_entries:
        lines.append(
            "No activities were crashed during the optimization process.")
        lines.append("")
    else:
        for entry in meaningful_entries:
            activity_id = entry.get('activity', '?')
            crash_cost = float(entry.get('cost', 0))
            # Use 'step_normal_cost' for the report, which is the sum of costs
            # for all active tasks in that step.
            ...
            lines.append(
                f"Step {
                    entry.get(
                        'iteration',
                        '?')}: Activity {activity_id} crashed to duration {
                    entry.get(
                        'duration',
                        '?')}")
            lines.append(f"  - Crash Cost: ${crash_cost:,.2f}")
            lines.append(
                f"  - Step Normal Cost (Active Tasks): ${step_normal_cost:,.2f}")
            lines.append(f"  - Step Total Cost: ${step_total_cost:,.2f}")
            lines.append(
                f"  - Cumulative Crash Cost: ${cumulative_crash_cost:,.2f}")
            lines.append(
                f"  - Cumulative Project Cost: ${cumulative_step_cost:,.2f}")
            lines.append(
                f"  - Project Duration: {entry.get('current_project_duration', '?')}")
            lines.append(
                f"  - Critical Path: {entry.get('critical_path', [])}")
            lines.append("")
    lines.append("=" * 50)
    ...
    if total_step_cost > 0:
        lines.append(
            f"Crash Cost %: {
                100 * result.total_crash_cost / total_step_cost:.2f}%")
        lines.append(
            f"Normal Cost %: {
                100 * result.total_normal_cost / total_step_cost:.2f}%")
    lines.append(
        f"Cost per Unit Duration Reduction: ${
            result.efficiency_metrics.get(
                'cost_per_unit_time',
                0):,.2f}")
    lines.append(f"Iterations Used: {result.iterations_used}")
    lines.append(f"Termination Reason: {result.termination_reason}")
    ...
    if result.crash_log:
        lines.append(
            f"Final Critical Path: {result.crash_log[-1].get('critical_path', [])}")
    ...
    if not activity_summary:
        ...
        for act, info in activity_summary.items():
            lines.append(
                f"  - Activity {act}: Crashed {
                    info['crash_count']}x, Total Crash Cost: ${
                    info['crash_cost']:,.2f}, Final Duration: {
                    info['final_duration']}")
    lines.append("=" * 50)
    return "\n".join(lines)
