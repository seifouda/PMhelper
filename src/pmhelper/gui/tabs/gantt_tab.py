...
class GanttTab:
    ...
    def create_control_frame(self):
        ...
        options_frame = ttk.LabelFrame(control_frame, text="Professional Gantt Chart Options", padding="5")
        options_frame.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 10))
        
        # Professional display options (critical path is always on, float removed)
        ...
        # Date settings frame
        date_frame = ttk.LabelFrame(control_frame, text="Project Dates", padding="5")
    ...
    def create_plot_area(self):
        ...
        try:
            ...
            error_label = ttk.Label(error_frame, text=f"Chart display error: {e}")
    ...
    def create_empty_plot(self):
        ...
        try:
            ...
            ax.set_title('Gantt Chart - No Data', fontsize=16, fontweight='bold')
            ax.set_xlim(0, 10)
            ax.set_ylim(0, 5)
            ax.axis('off')
                ...
                if node not in ['START', 'END']:
                    ...
                    for field in required_node_fields:
                        if field not in node_data:
                            print(f"ERROR: Node {node} missing required field: {field}")
                            self.create_error_chart(f"Invalid node data: missing {field}")
                            return False
    ...
    def update_gantt(self, results_data, analysis_mode):
        ...
        try:
            print(f"DEBUG: Updating professional Gantt chart with mode: {analysis_mode}")
            ...
            # Extract data from results - get the NetworkX graph for professional chart
            G = results_data.get('graph')
            critical_activities = results_data.get('critical_activities', [])
    ...
    def update_chart(self, G=None, critical_activities=None, project_duration=None):
        ...
        try:
            ...
            print(f"  Graph nodes: {len(G.nodes()) if G else 0}")
            print(f"  Critical activities: {len(critical_activities) if critical_activities else 0}")
            print(f"  Project duration: {project_duration}")
            ...
            if G and G.number_of_nodes() > 0:
                ...
                success = self.generate_professional_gantt_chart(G, critical_activities or [])
                ...
                self.create_error_chart("Failed to build project network graph")
                return False
    ...
    def generate_professional_gantt_chart(self, G, critical_activities):
        ...
        try:
            ...
            for node in G.nodes():
                if node not in ['START', 'END']:
                    ...
                    activity = {
                        ...
                        'name': node_data.get('activity', node),  # Ensure name field
                        'duration': node_data.get('duration', 0),
                        'earliest_start': node_data.get('earliest_start', 0),
                        'earliest_finish': node_data.get('earliest_finish', 0),
                        'latest_start': node_data.get('latest_start', 0),
            ...
            # Sort activities by earliest_start, then by ID (to match CPM Gantt)
            ...
            for i, activity in enumerate(activities):
                ...
                # Slack bar: from EF to LF (for non-critical, width=LF-EF, left=EF)
            ...
            if hasattr(self, 'show_predecessors_var') and self.show_predecessors_var.get():
            ...
            # Title styling
            ax.set_title('Project Gantt Chart', fontsize=14, fontweight='bold', pad=20)
            ...
            project_duration = max([a['latest_finish'] for a in activities]) if activities else 0
            if hasattr(self, 'show_today_var') and self.show_today_var.get():
                ...
                ax.axvline(x=today_position, color='green', linestyle='-', linewidth=2, 
                          alpha=0.8, label='Today')
            # Legend styling
            ...
            if hasattr(self, 'show_today_var') and self.show_today_var.get():
                ...
                legend_elements.append(mlines.Line2D([0], [0], color='green', linewidth=2, label='Today'))
            ...
            for activity in activities:
                ...
                for pred_id in predecessors:
                    if pred_id in activity_y_map and pred_id in activity_x_map:
                        ...
                        # Draw curved arrow
                        if abs(to_x - from_x) > 0.1 or abs(to_y - from_y) > 0.1:
                            ax.annotate('', xy=(to_x - 0.1, to_y), xytext=(from_x + 0.1, from_y),
    ...
    def save_chart(self):
        ...
        if not MATPLOTLIB_AVAILABLE or not self.results_data:
            messagebox.showwarning("Warning", "No chart to save. Please run analysis first.")
            return
    ...
    def export_schedule_data(self):
        ...
        if not self.results_data:
            messagebox.showwarning("Warning", "No schedule data to export. Please run analysis first.")
            return
        ...
        if filename:
            try:
                ...
                messagebox.showinfo("Success", f"Schedule data exported to {filename}")
            except Exception as e:
                messagebox.showerror("Error", f"Failed to export schedule data: {str(e)}")
    ...
    def _export_csv(self, filename):
        ...
        G = self.results_data.get('graph')
        critical_activities = set(self.results_data.get('critical_activities', []))
        data = []
        if G is not None:
            for node in G.nodes():
                ...
                duration = node_data.get('expected_duration', node_data.get('duration', 0))
                start_date = project_start + timedelta(days=es) if es else project_start
                finish_date = project_start + timedelta(days=ef) if ef else project_start
                preds = list(G.predecessors(node))
                preds_str = ', '.join(str(p) for p in preds)
                row = {
                    "Activity_ID": node,
                    "Activity_Name": node_data.get('activity', node),
                    "Duration": f"{duration:.2f}" if self.analysis_mode == 'probabilistic' else str(duration),
    ...
    def _export_excel(self, filename):
        ...
        try:
            ...
            messagebox.showerror("Error", "pandas is required for Excel export. Please install pandas.")
            return
        # Parse start date
        try:
            project_start = datetime.strptime(self.start_date_var.get(), "%Y-%m-%d")
