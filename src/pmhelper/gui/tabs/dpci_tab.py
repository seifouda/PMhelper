...
from ..models.dpci_model import DPCIAssessment, DPCICalculator, RiskLevel
...
class DPCITab(ttk.Frame):
    ...
    def _create_toolbar(self):
        ...
        ttk.Button(toolbar, text="New", command=self._new_assessment).pack(side='left', padx=2)
        ttk.Button(toolbar, text="Open", command=self._open_assessment).pack(side='left', padx=2)
        ttk.Button(toolbar, text="Save", command=self._save_assessment).pack(side='left', padx=2)
        ttk.Button(toolbar, text="Save As", command=self._save_as_assessment).pack(side='left', padx=2)
        
        ttk.Separator(toolbar, orient='vertical').pack(side='left', fill='y', padx=5)
        
        ttk.Button(toolbar, text="Export PDF", command=self._export_pdf).pack(side='left', padx=2)
        
        ttk.Separator(toolbar, orient='vertical').pack(side='left', fill='y', padx=5)
        
        ttk.Button(toolbar, text="Calculate", command=self._calculate_dpci).pack(side='left', padx=2)
    ...
    def _create_project_info(self):
        ...
        info_frame = ttk.LabelFrame(self.content_frame, text="Project Information", padding=10)
        info_frame.pack(fill='x', padx=10, pady=10)
        
        row = 0
        ttk.Label(info_frame, text="Project Name:").grid(row=row, column=0, sticky='w', pady=5)
        self.project_name_var = tk.StringVar()
        ttk.Entry(info_frame, textvariable=self.project_name_var, width=50).grid(row=row, column=1, sticky='ew', pady=5, padx=5)
        
        row += 1
        ttk.Label(info_frame, text="Assessed By:").grid(row=row, column=0, sticky='w', pady=5)
        self.assessed_by_var = tk.StringVar()
        ttk.Entry(info_frame, textvariable=self.assessed_by_var, width=50).grid(row=row, column=1, sticky='ew', pady=5, padx=5)
        
        row += 1
        ttk.Label(info_frame, text="Organization:").grid(row=row, column=0, sticky='w', pady=5)
        self.organization_var = tk.StringVar()
        ttk.Entry(info_frame, textvariable=self.organization_var, width=50).grid(row=row, column=1, sticky='ew', pady=5, padx=5)
        
        row += 1
        ttk.Label(info_frame, text="Description:").grid(row=row, column=0, sticky='nw', pady=5)
        self.description_text = tk.Text(info_frame, height=3, width=50, wrap='word')
        self.description_text.grid(row=row, column=1, sticky='ew', pady=5, padx=5)
    ...
    def _create_results_section(self):
        ...
        results_frame = ttk.LabelFrame(self.content_frame, text="Assessment Results", padding=10)
        ...
        # DPCI Index
        ttk.Label(results_frame, text="DPCI Index:", font=('TkDefaultFont', 10, 'bold')).grid(
            row=row, column=0, sticky='w', pady=5
        )
        self.dpci_label = ttk.Label(results_frame, text="Not calculated", font=('TkDefaultFont', 12))
        self.dpci_label.grid(row=row, column=1, sticky='w', pady=5, padx=10)
        
        row += 1
        
        # Risk Level
        ttk.Label(results_frame, text="Overall Risk Level:", font=('TkDefaultFont', 10, 'bold')).grid(
            row=row, column=0, sticky='w', pady=5
        )
        self.risk_label = ttk.Label(results_frame, text="Not calculated", font=('TkDefaultFont', 12))
        self.risk_label.grid(row=row, column=1, sticky='w', pady=5, padx=10)
        
        row += 1
        
        # Total Score
        ttk.Label(results_frame, text="Total Score:", font=('TkDefaultFont', 10, 'bold')).grid(
            row=row, column=0, sticky='w', pady=5
        )
        ...
        # Dimension scores
        ttk.Label(results_frame, text="Dimension Scores:", font=('TkDefaultFont', 10, 'bold')).grid(
            row=row, column=0, sticky='nw', pady=5
        )
        self.dimensions_label = ttk.Label(results_frame, text="Not calculated", justify='left')
        self.dimensions_label.grid(row=row, column=1, sticky='w', pady=5, padx=10)
        
        row += 1
        
        # Recommendations
        ttk.Label(results_frame, text="Recommendations:", font=('TkDefaultFont', 10, 'bold')).grid(
            row=row, column=0, sticky='nw', pady=5
        )
        ...
        self.recommendations_text = tk.Text(rec_frame, height=8, width=70, wrap='word', state='disabled')
    ...
    def _new_assessment(self):
        ...
        self.current_assessment = DPCIAssessment(assessment_id=str(uuid.uuid4()))
    ...
    def _save_assessment(self):
        ...
        if self.service.save_assessment(self.current_assessment, self.current_file):
    ...
    def _save_as_assessment(self):
        ...
        suggested_name = f"dpci_{self.current_assessment.project_name or 'assessment'}_{self.current_assessment.assessment_id[:8]}.json"
    ...
    def _export_pdf(self):
        ...
        if not DPCIPDFGenerator.is_available():
            messagebox.showerror("Error", "ReportLab library not installed. Cannot export to PDF.")
            return
        
        self._update_assessment_from_ui()
        
        suggested_name = f"dpci_report_{self.current_assessment.project_name or 'assessment'}_{self.current_assessment.assessment_id[:8]}.pdf"
        suggested_name = "".join(c for c in suggested_name if c.isalnum() or c in ('_', '-', '.'))
    ...
    def _update_assessment_from_ui(self):
        ...
        self.current_assessment.description = self.description_text.get('1.0', 'end-1c')
        ...
        for dim_key, criteria_vars in self.dimension_vars.items():
            ...
            # Update details
            details = {crit_key: var.get() for crit_key, var in criteria_vars.items()}
    ...
    def _load_assessment_to_ui(self):
        ...
        if self.current_assessment.description:
            self.description_text.insert('1.0', self.current_assessment.description)
