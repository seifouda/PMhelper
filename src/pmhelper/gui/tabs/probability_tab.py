...
def check_dependencies():
    ...
    try:
        ...
        import matplotlib
        if is_frozen:
            # Force TkAgg for frozen executables
...
class ProbabilityTab:
    ...
    def create_tab(self):
        ...
        ttk.Button(
            header_frame,
            text="? Help",
            command=self.main_window.show_probability_tab_help).pack(
            side=tk.RIGHT)

        # Create main layout with paned window
        self.paned_window = ttk.PanedWindow(
            self.probability_frame, orient=tk.HORIZONTAL)
    ...
    def create_dependencies_message(self):
        ...
        if getattr(sys, 'frozen', False):
            ...
            message_text = f"Probability analysis requires {
                ' and '.join(missing)}.\n"
            message_text += f"Please install {
                ' and '.join(missing)} to view probability analysis."
    ...
    def install_dependencies(self, packages):
        ...
        try:
            ...
            result = messagebox.askyesno(
                "Install Dependencies", f"This will install {
                    ' and '.join(packages)} using pip. Continue?")
            ...
            if result:
                ...
                messagebox.showinfo(
                    "Success", f"{
                        ' and '.join(packages)} installed successfully. Please restart the application.")
        except Exception as e:
    ...
    def create_statistics_frame(self, parent):
        ...
        stats_frame = ttk.LabelFrame(
            parent, text="Project Statistics", padding="10")
        ...
        self.variance_label = ttk.Label(
            stats_frame, text="Project Variance: --")
        self.variance_label.pack(anchor="w", pady=2)

        self.std_deviation_label = ttk.Label(
            stats_frame, text="Standard Deviation: --")
        self.std_deviation_label.pack(anchor="w", pady=2)

        self.confidence_interval_label = ttk.Label(
            stats_frame, text="95% Confidence Interval: --")
        ...
        self.critical_path_label = ttk.Label(
            stats_frame, text="Critical Path Length: --")
        self.critical_path_label.pack(anchor="w", pady=2)

        self.critical_variance_label = ttk.Label(
            stats_frame, text="Critical Path Variance: --")
    ...
    def create_calculator_frame(self, parent):
        ...
        calc_frame = ttk.LabelFrame(
            parent,
            text="Completion Probability Calculator",
            padding="10")
        ...
        # Calculate Probability button (moved to right of target duration
        # field)
        ttk.Button(
            target_frame,
            text="Calculate Probability",
            command=self.calculate_completion_probability).pack(
            side=tk.LEFT,
            padx=(
                10,
                0))

        # Target percentage input and Calculate Duration button (row 2, just
        # below target duration)
        ...
        percentage_entry = ttk.Entry(
            percent_frame,
            textvariable=self.target_percentage_var,
            width=6)
        ...
        ttk.Button(
            ...
            command=self.calculate_duration_for_probability).pack(
            side=tk.LEFT,
            padx=(
                10,
                0))

        # Clear Fields button (replaces old Calculate Probability button above
        # results)
        ttk.Button(
            percent_frame,
            text="Clear Fields",
            command=self.clear_calculator_fields).pack(
            pady=5,
            side=tk.RIGHT)

        # Results display
        self.probability_result_label = ttk.Label(
            calc_frame,
        ...
        scenarios_frame = ttk.LabelFrame(
            calc_frame, text="Common Scenarios", padding="5")
        ...
        ttk.Button(
            ...
            text="Expected",
            width=10,
            command=lambda: self.quick_probability("expected")).pack(
            side=tk.LEFT,
            padx=2)
        ttk.Button(
            button_row,
            text="+1σ",
            width=10,
            command=lambda: self.quick_probability("plus_1sigma")).pack(
            side=tk.LEFT,
            padx=2)
        ttk.Button(
            button_row,
            text="-1σ",
            width=10,
            command=lambda: self.quick_probability("minus_1sigma")).pack(
            side=tk.LEFT,
            padx=2)
        ttk.Button(
            button_row,
            text="+2σ",
            width=10,
            command=lambda: self.quick_probability("plus_2sigma")).pack(
            side=tk.LEFT,
            padx=2)
        ttk.Button(
            button_row,
            text="-2σ",
            width=10,
            command=lambda: self.quick_probability("minus_2sigma")).pack(
            side=tk.LEFT,
            padx=2)
    ...
    def create_risk_frame(self, parent):
        ...
        self.schedule_risk_label = ttk.Label(
            risk_frame, text="Schedule Risk: --")
        ...
        ttk.Button(
            ...
            command=self.export_probability_analysis).pack(
            side=tk.LEFT,
            padx=(
                0,
                5))
    ...
    def create_plot_area(self):
        ...
        ttk.Button(
            chart_frame,
            text="Update Chart",
            command=self.update_visualization).pack(
            side=tk.LEFT,
            padx=10)
        ...
        if not _lazy_import_dependencies():
            ttk.Label(
                plot_frame,
                text="Visualization dependencies not available").pack()
            return
    ...
    def update_analysis(self, results_data):
        ...
        try:
            ...
            print(
                f"       Activities: {len(results_data.get('activities', []))}")
            print(
                f"       Project Duration: {
                    results_data.get(
                        'project_duration',
    ...
    def update_statistics(self):
        ...
        self.expected_duration_label.config(
            text=f"Expected Duration: {
                expected_duration:.2f} days")
        self.variance_label.config(text=f"Project Variance: {variance:.3f}")
        self.std_deviation_label.config(
            text=f"Standard Deviation: {
                std_deviation:.2f} days")
        ...
        critical_variance = sum(a.get('variance', 0)
        ...
        self.critical_path_label.config(
            text=f"Critical Path Length: {
                len(critical_path)} activities    [Critical Path: {path_str}]")
        self.critical_variance_label.config(
            text=f"Critical Path Variance: {
                critical_variance:.3f}")
    ...
    def calculate_completion_probability(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Error", "Please enter a valid target duration.")
            return
    ...
    def calculate_duration_for_probability(self):
        ...
        duration = stats.norm.ppf(
            probability,
            expected_duration,
            std_deviation)

        self.duration_result_label.config(
            text=f"Required Duration: {
                duration:.1f} days for {
                percentage:.1f}% probability")
    ...
    def create_distribution_chart(self, target_duration=None):
        ...
        if target_duration is not None:
            ax.axvline(
                ...
                linestyle='-',
                linewidth=2,
                label=f'Target Duration: {
                    target_duration:.1f}')

            # Shade area to the left of target (completion probability)
            x_target = x[x <= target_duration]
            y_target = stats.norm.pdf(
                x_target, expected_duration, std_deviation)
    ...
    def create_sensitivity_chart(self):
        ...
        ax.set_title(
            'Activity Variance Sensitivity Analysis\n(Critical Path Activities)')
        ...
        for _ in range(n_simulations):
            ...
            for activity in critical_activities:
                ...
                if optimistic and most_likely and pessimistic:
                    ...
                    alpha = 1 + 4 * (most_likely - optimistic) / \
                        (pessimistic - optimistic)
                    beta = 1 + 4 * (pessimistic - most_likely) / \
                        (pessimistic - optimistic)
    ...
    def update_risk_analysis(self):
        ...
        for i, activity in enumerate(high_risk_activities):
            ...
            risk_text += f"{i +
                            1}. {activity.get('id', '')} - {activity.get('name', '')}\n"
    ...
    def export_probability_analysis(self):
        ...
        filename = filedialog.asksaveasfilename(
            title="Export Probability Analysis", defaultextension=".csv", filetypes=[
        ...
        if filename:
            try:
                ...
                messagebox.showinfo(
                    "Success", f"Analysis exported to {filename}")
            except Exception as e:
    ...
    def generate_risk_report(self):
        ...
        ttk.Button(
            button_frame,
            text="Save Report",
            command=lambda: self._save_report(report_content)).pack(
            side=tk.RIGHT)
    ...
    def _generate_report_content(self):
        ...
        report += f"Coefficient of Variation: {(std_deviation /
                                                expected_duration) *
                                               100:.1f}%\n\n"
    ...
    def _save_report(self, content):
        ...
        if filename:
            try:
                ...
                messagebox.showerror(
                    "Error", f"Failed to save report: {
                        str(e)}")

    def show(self):
        """Show the probability tab"""
        # Add the tab to the notebook if it's not already there
        if self.probability_frame not in [
                self.notebook.nametowidget(tab) for tab in self.notebook.tabs()]:
            self.notebook.add(
