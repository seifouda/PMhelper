...
class RCPSProjectCrashing(ProjectCrashing):
    ...
    def __init__(self, rcps_analyzer, resource_limit):
        """
        Initialize RCPS Project Crashing with analyzer (same pattern as parent)
        
        Args:
            rcps_analyzer: RCPSAnalyzer instance with RCPS network data
            resource_limit: Maximum available resources
        """
        super().__init__(rcps_analyzer)  # Parent expects analyzer
        self.resource_limit = resource_limit
        self.rcps_analyzer = rcps_analyzer
        
        print(f"[DEBUG] RCPSProjectCrashing initialized with resource_limit={resource_limit}")
    
    def run(self, target_duration, strategy, objective, **kwargs):
        """Override to add RCPS-specific resource validation"""
        # Call parent run method
        result = super().run(target_duration, strategy, objective, **kwargs)
        
        # Add RCPS-specific post-processing if needed
        if result:
            result.resource_limit = self.resource_limit
            result.resource_feasible = self._validate_final_schedule()
        
        return result
        
    def _validate_final_schedule(self):
        """Validate that final crashed schedule respects resource constraints"""
        return self.rcps_analyzer.validate_resource_constraints()
