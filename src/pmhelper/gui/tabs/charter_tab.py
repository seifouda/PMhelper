...
class CharterTab(ttk.Frame):
    ...
    def _create_toolbar(self):
        ...
        self._create_tooltip(self.new_btn, "Create a new project charter (Ctrl+N)")
        ...
        self._create_tooltip(self.open_btn, "Open an existing charter (Ctrl+O)")
        ...
        self._create_tooltip(self.help_btn, "Show help and keyboard shortcuts (F1)")
    ...
    def _create_content_area(self):
        ...
        self.welcome_label.grid(row=0, column=0, sticky="nsew", padx=20, pady=20)
    ...
    def _on_new_charter(self):
        ...
        self._update_status("TODO: Implement template selection and charter creation")
    ...
    def _create_new_charter(self, template_id: str):
        ...
        if not template:
            self._update_status(f"Error: Could not load template '{template_id}'")
            return
    ...
    def _on_open_charter(self):
        ...
        # Load template
        template = self.template_service.load_template(charter.metadata.template_id)
        if not template:
    ...
    def _on_save_charter(self):
        ...
        if self.current_filepath:
            ...
            if self.storage_service.save_charter(self.current_charter, self.current_filepath):
                self.last_saved_time = datetime.now()
    ...
    def _save_charter_with_dialog(self) -> bool:
        ...
        project_name = self.current_charter.get_field_value('identification', 'project_name')
        if project_name:
    ...
    def _on_export_pdf(self):
        ...
        if not self.current_form.validate():
            ...
            response = messagebox.askyesno(
                "Validation Errors",
                f"Charter has {len(errors)} validation error(s).\n\nExport anyway?"
            )
            if not response:
                return
    ...
    def _update_progress(self):
        ...
        if self.current_charter and self.current_template:
            completion = self.current_charter.get_completion_percentage(self.current_template)
            self.progress_label.configure(text=f"Completion: {completion:.0f}%")
        else:
    ...
    def _has_unsaved_changes(self) -> bool:
        ...
        modified_time = datetime.fromisoformat(self.current_charter.metadata.modified_date)
    ...
    def _auto_save(self):
        ...
        try:
            if self.current_charter and self.current_form:
                ...
                if self.storage_service.save_charter(self.current_charter, draft_path):
                    self.last_saved_time = datetime.now()
                    # Update status in main thread
                    self.after(0, lambda: self._update_status("Auto-saved to drafts"))
    ...
    def _update_last_saved_display(self):
        ...
        if self.last_saved_time:
            ...
            self.progress_label.configure(
                text=f"Completion: {self._get_completion():.0f}% | Last saved: {time_str}"
            )
        else:
            self.progress_label.configure(
    ...
    def _get_completion(self) -> float:
        ...
        if self.current_charter and self.current_template:
            return self.current_charter.get_completion_percentage(self.current_template)
        return 0.0
