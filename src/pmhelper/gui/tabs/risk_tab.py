...
class RiskAnalysisTab:
    ...
    def create_tab(self):
        ...
        title_label = ttk.Label(header_frame, text="Comprehensive Risk Analysis", 
                               font=("Arial", 12, "bold"))
...
IM 738 course material."""
    ...
    def create_delay_results_panel(self, parent):
        ...
        # Results display
        results_frame = ttk.LabelFrame(parent, text="Risk Assessment", padding="10")
        ...
        # Scrollbar
        scrollbar = ttk.Scrollbar(results_frame, command=self.delay_results_text.yview)
    ...
    def calculate_delay_risk(self):
        ...
        try:
            ...
            if not hasattr(self.main_window.pert_analyzer, 'G') or self.main_window.pert_analyzer.G is None:
                messagebox.showerror("Error", "No analysis results available. Please run PERT analysis first!")
                return
    ...
    def display_delay_risk_results(self, risk, contract_time, penalty_rate):
        ...
        self.delay_results_text.insert(tk.END, "DELAY RISK ANALYSIS RESULTS\n", "header")
        self.delay_results_text.insert(tk.END, "=" * 50 + "\n\n", "header")
        
        # Parameters
        self.delay_results_text.insert(tk.END, f"Contract Time: {contract_time:.2f} weeks\n")
        self.delay_results_text.insert(tk.END, f"Penalty Rate: ${penalty_rate:,.2f}/week\n\n")
        
        # Results
        delay_prob = risk['delay_probability']
        self.delay_results_text.insert(tk.END, f"Delay Probability: {delay_prob:.1%}\n")
        ...
        self.delay_results_text.insert(tk.END, f"Risk Level: {risk_level}\n\n", tag)
        
        self.delay_results_text.insert(tk.END, 
            f"Expected Delay: {risk['expected_delay']:.2f} weeks\n")
        self.delay_results_text.insert(tk.END, 
            f"Risk Cost: ${risk['risk_cost']:,.2f}\n")
        
        if risk['max_penalty'] is not None:
            self.delay_results_text.insert(tk.END, 
                f"Capped Risk Cost: ${risk['capped_risk_cost']:,.2f}\n")
            self.delay_results_text.insert(tk.END, 
                f"Max Penalty: ${risk['max_penalty']:,.2f}\n")
    ...
    def create_contingency_input_panel(self, parent):
        ...
        quick_frame = ttk.LabelFrame(parent, text="Quick Calculate", padding="10")
        quick_frame.pack(fill=tk.X, padx=5, pady=5)
        
        ttk.Button(quick_frame, text="80%", width=8,
                  command=lambda: self.quick_contingency(0.80)).pack(side=tk.LEFT, padx=2)
        ttk.Button(quick_frame, text="90%", width=8,
                  command=lambda: self.quick_contingency(0.90)).pack(side=tk.LEFT, padx=2)
        ttk.Button(quick_frame, text="95%", width=8,
                  command=lambda: self.quick_contingency(0.95)).pack(side=tk.LEFT, padx=2)
        ttk.Button(quick_frame, text="99%", width=8,
                  command=lambda: self.quick_contingency(0.99)).pack(side=tk.LEFT, padx=2)
...
Standard: 90-95% for most projects"""
    ...
    def update_confidence_label(self, value=None):
        ...
        self.confidence_label.config(text=f"{self.confidence_var.get()*100:.0f}%")
    ...
    def create_contingency_results_panel(self, parent):
        ...
        results_frame = ttk.LabelFrame(parent, text="Contingency Estimates", padding="10")
        results_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        # Create text widget
        self.contingency_results_text = tk.Text(results_frame, height=15, width=50,
                                               font=("Courier", 10))
        self.contingency_results_text.pack(fill=tk.BOTH, expand=True)
        
        scrollbar = ttk.Scrollbar(results_frame, command=self.contingency_results_text.yview)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        self.contingency_results_text.config(yscrollcommand=scrollbar.set)
        
        # Tags
        self.contingency_results_text.tag_config("header", font=("Courier", 10, "bold"))
        self.contingency_results_text.tag_config("highlight", background="yellow")
    ...
    def calculate_contingency(self):
        ...
        try:
            ...
            daily_cost = float(self.daily_cost_var.get()) if self.daily_cost_var.get() else None
    ...
    def display_contingency_results(self, result):
        ...
        self.contingency_results_text.insert(tk.END, "CONTINGENCY PLANNING RESULTS\n", "header")
        self.contingency_results_text.insert(tk.END, "=" * 50 + "\n\n", "header")
        
        # Results
        self.contingency_results_text.insert(tk.END, 
            f"Confidence Level: {result['confidence_level']*100:.0f}%\n")
        self.contingency_results_text.insert(tk.END, 
            f"Z-Score: {result['z_score']:.3f}\n\n")
        
        self.contingency_results_text.insert(tk.END, 
            f"Time Buffer Required: {result['time_buffer']:.2f} weeks\n", "highlight")
        self.contingency_results_text.insert(tk.END, 
            f"Buffer Percentage: {result['buffer_percentage']:.1f}%\n")
        self.contingency_results_text.insert(tk.END, 
            f"\nCompletion Time: {result['completion_time']:.2f} weeks\n", "highlight")
        
        if result['contingency_cost'] is not None:
            self.contingency_results_text.insert(tk.END, 
                f"Contingency Cost: ${result['contingency_cost']:,.2f}\n")
        
        # Recommendation
        self.contingency_results_text.insert(tk.END, "\n" + "-" * 50 + "\n", "header")
        self.contingency_results_text.insert(tk.END, "RECOMMENDATION\n", "header")
        self.contingency_results_text.insert(tk.END, "-" * 50 + "\n\n", "header")
        self.contingency_results_text.insert(tk.END, result['recommendation'] + "\n")
    ...
    def create_strategies_input_panel(self, parent):
        ...
        row = 0
        # Contract time
        ttk.Label(inputs_frame, text="Contract Time:").grid(row=row, column=0, sticky=tk.W, pady=3)
        self.strat_contract_var = tk.StringVar(value="15.0")
        ttk.Entry(inputs_frame, textvariable=self.strat_contract_var, width=12).grid(
            row=row, column=1, pady=3)
        
        row += 1
        # Penalty rate
        ttk.Label(inputs_frame, text="Penalty Rate:").grid(row=row, column=0, sticky=tk.W, pady=3)
        self.strat_penalty_var = tk.StringVar(value="1000")
        ttk.Entry(inputs_frame, textvariable=self.strat_penalty_var, width=12).grid(
            row=row, column=1, pady=3)
        
        row += 1
        # Time reduction cost
        ttk.Label(inputs_frame, text="Time Reduction Cost:").grid(row=row, column=0, sticky=tk.W, pady=3)
        self.time_cost_var = tk.StringVar(value="3000")
        ttk.Entry(inputs_frame, textvariable=self.time_cost_var, width=12).grid(
            row=row, column=1, pady=3)
        
        row += 1
        # Variance reduction cost
        ttk.Label(inputs_frame, text="Variance Reduction Cost:").grid(row=row, column=0, sticky=tk.W, pady=3)
        ...
        row += 1
        # Max budget
        ttk.Label(inputs_frame, text="Max Budget:").grid(row=row, column=0, sticky=tk.W, pady=3)
...
Compares ROI and net benefit"""
    ...
    def create_strategies_results_panel(self, parent):
        ...
        results_frame = ttk.LabelFrame(parent, text="Strategy Comparison", padding="10")
        results_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        self.strategies_results_text = tk.Text(results_frame, height=15, width=50,
                                              font=("Courier", 9))
        self.strategies_results_text.pack(fill=tk.BOTH, expand=True)
        
        scrollbar = ttk.Scrollbar(results_frame, command=self.strategies_results_text.yview)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        self.strategies_results_text.config(yscrollcommand=scrollbar.set)
        
        self.strategies_results_text.tag_config("header", font=("Courier", 9, "bold"))
        self.strategies_results_text.tag_config("best", background="lightgreen")
    ...
    def display_strategies_results(self, result):
        ...
        self.strategies_results_text.insert(tk.END, "VARIANCE REDUCTION STRATEGY COMPARISON\n", "header")
        self.strategies_results_text.insert(tk.END, "=" * 60 + "\n\n", "header")
        
        # Baseline
        self.strategies_results_text.insert(tk.END, 
            f"Baseline Risk Cost: ${result['baseline']['risk_cost']:,.2f}\n\n")
        
        # Strategy A
        self.strategies_results_text.insert(tk.END, "STRATEGY A (Reduce Time)\n", "header")
        self.strategies_results_text.insert(tk.END, "-" * 60 + "\n")
        sa = result['strategy_a']
        self.strategies_results_text.insert(tk.END, 
            f"  Time Reduction: {sa['time_reduction']:.2f} weeks\n")
        self.strategies_results_text.insert(tk.END, 
            f"  Investment: ${sa['investment']:,.2f}\n")
        self.strategies_results_text.insert(tk.END, 
            f"  Benefit: ${sa['benefit']:,.2f}\n")
        self.strategies_results_text.insert(tk.END, 
            f"  Net Benefit: ${sa['net_benefit']:,.2f}\n")
        self.strategies_results_text.insert(tk.END, 
            f"  ROI: {sa['roi']:.1f}%\n\n")
        
        # Strategy B
        self.strategies_results_text.insert(tk.END, "STRATEGY B (Reduce Variance)\n", "header")
        self.strategies_results_text.insert(tk.END, "-" * 60 + "\n")
        sb = result['strategy_b']
        self.strategies_results_text.insert(tk.END, 
            f"  Variance Reduction: {sb['variance_reduction']:.2f}\n")
        self.strategies_results_text.insert(tk.END, 
            f"  Investment: ${sb['investment']:,.2f}\n")
        self.strategies_results_text.insert(tk.END, 
            f"  Benefit: ${sb['benefit']:,.2f}\n")
        self.strategies_results_text.insert(tk.END, 
            f"  Net Benefit: ${sb['net_benefit']:,.2f}\n")
        self.strategies_results_text.insert(tk.END, 
            f"  ROI: {sb['roi']:.1f}%\n\n")
        
        # Mixed Strategy
        self.strategies_results_text.insert(tk.END, "MIXED STRATEGY\n", "header")
        self.strategies_results_text.insert(tk.END, "-" * 60 + "\n")
        sm = result['mixed_strategy']
        self.strategies_results_text.insert(tk.END, 
            f"  Time Reduction: {sm['time_reduction']:.2f} weeks\n")
        self.strategies_results_text.insert(tk.END, 
            f"  Variance Reduction: {sm['variance_reduction']:.2f}\n")
        self.strategies_results_text.insert(tk.END, 
            f"  Investment: ${sm['investment']:,.2f}\n")
        self.strategies_results_text.insert(tk.END, 
            f"  Benefit: ${sm['benefit']:,.2f}\n")
        self.strategies_results_text.insert(tk.END, 
            f"  Net Benefit: ${sm['net_benefit']:,.2f}\n")
        self.strategies_results_text.insert(tk.END, 
            f"  ROI: {sm['roi']:.1f}%\n\n")
        ...
        self.strategies_results_text.insert(tk.END, "=" * 60 + "\n", "header")
        self.strategies_results_text.insert(tk.END, "✓ RECOMMENDED STRATEGY\n", "header")
        self.strategies_results_text.insert(tk.END, "=" * 60 + "\n")
        self.strategies_results_text.insert(tk.END, 
            f"{best['name']}\n", "best")
        self.strategies_results_text.insert(tk.END, 
            f"Best Net Benefit: ${best['net_benefit']:,.2f}\n\n", "best")
    ...
    def create_activity_risks_tab(self):
        ...
        ttk.Button(control_frame, text="Calculate Risk Scores",
                  command=self.calculate_activity_risks).pack(side=tk.LEFT, padx=5)
        
        ttk.Button(control_frame, text="Generate Mitigation Plan",
                  command=self.generate_mitigation_plan).pack(side=tk.LEFT, padx=5)
        
        ttk.Button(control_frame, text="Export to CSV",
                  command=self.export_activity_risks).pack(side=tk.LEFT, padx=5)
    ...
    def calculate_activity_risks(self):
        ...
        try:
            ...
            for idx, row in df.iterrows():
                ...
                self.activity_tree.insert('', tk.END, values=values, tags=(tag,))
            
            messagebox.showinfo("Success", 
                f"Risk scores calculated for {len(df)} activities")
    ...
    def export_activity_risks(self):
        ...
        try:
            if self.activity_risks_data is None:
                messagebox.showwarning("Warning", "Please calculate risk scores first!")
                return
