...
try:
    ...
    from pmhelper.utils.plotly_charts import (
        plotly_time_cost_curve, plotly_resource_profiles,
        plotly_pareto_frontier, PLOTLY_AVAILABLE as _PLT_AVAIL,
    )
...
class OptimizationTab(ttk.Frame):
    ...
    def __init__(self, parent, cpm_analyzer=None):
        ...
        # Create sub-tabs
        self.time_cost_tab = TimeCostOptimizationPanel(
            self.notebook, self.cpm_analyzer)
        self.resource_tab = ResourceLevelingPanel(
            self.notebook, self.cpm_analyzer)
        self.multi_obj_tab = MultiObjectivePanel(
            self.notebook, self.cpm_analyzer)
...
class TimeCostOptimizationPanel(ttk.Frame):
    ...
    def create_input_panel(self):
        ...
        cost_frame = ttk.LabelFrame(
            self.left_frame,
            text="Indirect Costs (Daily Rates)",
            padding=10)
        ...
        results_frame = ttk.LabelFrame(
            self.left_frame, text="Results", padding=10)
    ...
    def create_visualization_panel(self):
        ...
        # Matplotlib frame
        self._mpl_viz_frame = ttk.Frame(viz_frame)
        self._mpl_viz_frame.pack(fill='both', expand=True)
        ...
        # Create canvas
        self.canvas = FigureCanvasTkAgg(self.fig, self._mpl_viz_frame)
        self.canvas.draw()
        self.canvas.get_tk_widget().pack(fill='both', expand=True)

        # Add toolbar
        toolbar_frame = ttk.Frame(self._mpl_viz_frame)
    ...
    def plot_curve(self, curve_data, result):
        ...
        # Store for Plotly re-render
        if _PLOTLY_EMBED:
            self._last_curve = curve_data
            self._last_result = result
            if self._render_mode_var.get() == "plotly":
                self._update_plotly_tc()
                return
    ...
    # -- Plotly helpers (TimeCost) ------------------------------------------
    def _switch_renderer(self):
        if not _PLOTLY_EMBED:
            return
        if self._render_mode_var.get() == "plotly":
            self._mpl_viz_frame.pack_forget()
            self._plotly_frame.pack(fill="both", expand=True)
            if self._last_curve and self._last_result:
                self._update_plotly_tc()
        else:
            self._plotly_frame.pack_forget()
            self._mpl_viz_frame.pack(fill="both", expand=True)

    def _update_plotly_tc(self):
        if not self._last_curve or not self._last_result:
            return
        try:
            fig = plotly_time_cost_curve(self._last_curve, self._last_result)
            self._plotly_frame.update_chart(fig)
        except Exception as exc:
            logger.error("Plotly time-cost render error: %s", exc)
...
class ResourceLevelingPanel(ttk.Frame):
    ...
    def create_control_panel(self):
        ...
        method_frame = ttk.LabelFrame(
            self.left_frame, text="Leveling Method", padding=10)
        method_frame.pack(fill='x', padx=5, pady=5)

        self.method_var = tk.StringVar(value='minimum_moment')
        ttk.Radiobutton(
            method_frame,
            text="Minimum Moment Algorithm",
            variable=self.method_var,
            value='minimum_moment').pack(
            anchor='w')
        ttk.Radiobutton(
            method_frame,
            text="Burgess Method",
        ...
        limit_frame = ttk.LabelFrame(
            self.left_frame,
            text="Resource Constraint (Optional)",
            padding=10)
        limit_frame.pack(fill='x', padx=5, pady=5)

        ttk.Label(
            limit_frame,
            text="Max Resource Units:").grid(
            row=0,
            column=0,
            sticky='w')
        ...
        ttk.Label(
            limit_frame,
            text="(Leave blank for unconstrained)",
            font=(
                'Arial',
                8,
                'italic')).grid(
        ...
        # Results Display
        results_frame = ttk.LabelFrame(
            self.left_frame, text="Metrics", padding=10)
        results_frame.pack(fill='both', expand=True, padx=5, pady=5)
    ...
    def run_leveling(self):
        ...
        try:
            ...
            leveler = ResourceLevelingFactory.create(
                method, activities, resource_limit)
    ...
    def display_metrics(self, result):
        ...
        original_avg = 0.0
        leveled_avg = 0.0
        if 'original_profile' in result and hasattr(
                result['original_profile'], 'profile'):
            orig_values = list(result['original_profile'].profile.values())
            original_avg = sum(orig_values) / \
                len(orig_values) if orig_values else 0.0
        if 'leveled_profile' in result and hasattr(
                result['leveled_profile'], 'profile'):
            lev_values = list(result['leveled_profile'].profile.values())
            leveled_avg = sum(lev_values) / \
                len(lev_values) if lev_values else 0.0
    ...
    # -- Plotly helpers (Resource Leveling) ----------------------------------
    def _switch_renderer(self):
        if not _PLOTLY_EMBED:
            return
        if self._render_mode_var.get() == "plotly":
    ...
    def _update_plotly_rl(self):
        if not self._last_result:
            return
        try:
            fig = plotly_resource_profiles(self._last_result)
            self._plotly_frame.update_chart(fig)
        except Exception as exc:
            logger.error("Plotly resource-leveling render error: %s", exc)
...
class MultiObjectivePanel(ttk.Frame):
    ...
    def create_control_panel(self):
        ...
        obj_frame = ttk.LabelFrame(
            self.left_frame, text="Objectives", padding=10)
        ...
        # Results
        results_frame = ttk.LabelFrame(
            self.left_frame, text="Pareto Solutions", padding=10)
        results_frame.pack(fill='both', expand=True, padx=5, pady=5)
    ...
    def create_visualization_panel(self):
        ...
        if _PLOTLY_EMBED:
            self._plotly_frame = PlotlyChartFrame(viz_frame)
            self._last_solutions_df = None
            self._last_objs = ("duration", "cost")
    ...
    def run_optimization(self):
        ...
        try:
            ...
            selected = [
                name for name,
                var in self.objectives.items() if var.get()]
            ...
            if 'npv' in selected:
                objectives['npv'] = lambda s: - \
                    calc_npv(s)  # Negative to minimize

            # Create optimizer
            optimizer = MultiObjectiveOptimizer(objectives)

            # Generate schedules
            schedules = ScheduleGenerator.generate_sampled(
                activities, num_samples)

            # Compute Pareto frontier
            minimize_objs = ['duration', 'cost', 'npv']  # All minimize
            frontier = optimizer.generate_solutions_grid(
                schedules, minimize_objs)
    ...
    def plot_frontier(self, solutions_df, obj1, obj2):
        ...
        # Store for Plotly re-render
        if _PLOTLY_EMBED:
            self._last_solutions_df = solutions_df
            self._last_objs = (obj1, obj2)
            if self._render_mode_var.get() == "plotly":
                self._update_plotly_pareto()
                return
    ...
    # -- Plotly helpers (Multi-Objective) ------------------------------------
    def _switch_renderer(self):
        if not _PLOTLY_EMBED:
            return
        if self._render_mode_var.get() == "plotly":
            self._mpl_viz_frame.pack_forget()
            self._plotly_frame.pack(fill="both", expand=True)
            if self._last_solutions_df is not None:
                self._update_plotly_pareto()
        else:
            self._plotly_frame.pack_forget()
            self._mpl_viz_frame.pack(fill="both", expand=True)

    def _update_plotly_pareto(self):
        if self._last_solutions_df is None:
            return
        try:
            obj1, obj2 = self._last_objs
            fig = plotly_pareto_frontier(self._last_solutions_df, obj1, obj2)
            if fig:
                self._plotly_frame.update_chart(fig)
        except Exception as exc:
            logger.error("Plotly pareto render error: %s", exc)
