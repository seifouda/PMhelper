...
class RCPSCrashingTabGUIManager(CrashingTabGUIManager):
    ...
    def __init__(self, tab_instance, app_instance):
        ...
        # Add duration info banner at the top
        self._duration_info_var = tk.StringVar(
            value="Original CPM Duration: —  |  RCPS Duration: —  |  Run RCPS analysis first")
        self._duration_banner = ttk.Label(
            self.tab, textvariable=self._duration_info_var,
            font=("TkDefaultFont", 10, "bold"), foreground="#2c3e50",
            background="#ecf0f1", relief="groove", padding=(8, 4))
        # Pack at top, before controls frame
        self._duration_banner.pack(
            fill=tk.X, padx=5, pady=(
                5, 0), before=self.tab.winfo_children()[0])
    ...
    def _recalculate_critical_path_for_step(self, G_step, step_data=None):
            ...
            if rcps_analyzer:
                ...
                for node in G_step.nodes():
                    if node in G_work.nodes():
                        G_step.nodes[node]['ES'] = G_work.nodes[node].get(
                            'ES', G_step.nodes[node].get('ES', 0))
                        G_step.nodes[node]['EF'] = G_work.nodes[node].get(
                            'EF', G_step.nodes[node].get('EF', 0))
                        G_step.nodes[node]['LS'] = G_work.nodes[node].get(
                            'LS', G_step.nodes[node].get('LS', 0))
                        G_step.nodes[node]['LF'] = G_work.nodes[node].get(
                            'LF', G_step.nodes[node].get('LF', 0))
                        G_step.nodes[node]['float'] = G_work.nodes[node].get(
                            'float', G_step.nodes[node].get('float', 1))

                print(
                    "[DEBUG] RCPS: Applied RCPS CPM calculation successfully")
                return G_step
        ...
        if step_data and isinstance(step_data,
                                    dict) and step_data.get('critical_path'):
            cp = step_data.get('critical_path', [])
            for n in G_step.nodes():
                G_step.nodes[n]['float'] = 0 if n in cp else G_step.nodes[n].get(
                    'float', 1)
            print("[DEBUG] RCPS: Used critical path from crash log")
            return G_step
        ...
        try:
            ...
            if network_builder:
                ...
                print("[DEBUG] RCPS: Applied CPM fallback calculation")
        except Exception as e:
    ...
    def update_visualization(self, result):
        ...
        # RCPS enhancement: Post-process step graphs to apply proper critical
        # path calculation
        if hasattr(self, 'step_graphs') and self.step_graphs:
            print(
                f"[DEBUG] RCPS: Enhancing {len(self.step_graphs)} step graphs with RCPS critical path")

            enhanced_step_graphs = []
            for i, (step_num, activity, new_duration,
                ...
                if step_num > 0:  # Skip initial step (no changes needed)
                    ...
                    if result and hasattr(
                            result, 'crash_log') and result.crash_log:
                        if (step_num - 1) < len(result.crash_log):
                            step_data = result.crash_log[step_num - 1]

                    # Apply RCPS-specific critical path calculation
                    enhanced_G_step = self._recalculate_critical_path_for_step(
                        G_step.copy(), step_data)
                    enhanced_step_graphs.append(
                        (step_num, activity, new_duration, enhanced_G_step))
                else:
                    # Keep initial step as-is
                    enhanced_step_graphs.append(
                        (step_num, activity, new_duration, G_step))

            # Replace step graphs with enhanced versions
            self.step_graphs = enhanced_step_graphs
            print(
                "[DEBUG] RCPS: Enhanced step graphs with RCPS critical path calculation")
                ...
                for child in widget.winfo_children():
                    if isinstance(child, ttk.Button):
                        if child.cget("text") == "? Help":
                            child.configure(
                                command=self.app.show_rcps_crashing_tab_help)
    ...
    def _update_interface_for_rcps(self):
        ...
        try:
            ...
            for widget in self.tab.winfo_children():
                if isinstance(
                        widget,
                        ttk.LabelFrame) and "Crashing Controls" in widget.cget("text"):
            ...
            for widget in self.tab.winfo_children():
                if isinstance(
                        widget,
                        ttk.LabelFrame) and "Crashing Results" in widget.cget("text"):
            ...
            for widget in self.tab.winfo_children():
                if isinstance(
                        widget,
                        ttk.LabelFrame) and "Crashing Visualization" in widget.cget("text"):
...
"""
    ...
    def _setup_enhanced_step_navigation(self):
        ...
        try:
            ...
            if hasattr(self, 'step_select_spinbox'):
                self.step_select_spinbox.bind(
                    '<Return>', self._on_step_entry_enter)
                self.step_select_spinbox.bind(
                    '<KP_Enter>', self._on_step_entry_enter)  # Numpad Enter
                print("[DEBUG] Enhanced step navigation setup completed")
        except Exception as e:
            print(
                f"[DEBUG] Error setting up enhanced step navigation: {
                    str(e)}")

    def on_tab_selected(self):
        """Refresh duration info banner when tab is selected."""
        cpm_dur = "—"
        rcps_dur = "—"
        try:
            analyzer = getattr(self.app, 'cpm_analyzer', None)
            if analyzer and hasattr(
                    analyzer,
                    'project_duration') and analyzer.project_duration:
                cpm_dur = f"{analyzer.project_duration:.0f}"
        except Exception:
            pass
        try:
            if (self.rcps_tab
                    and hasattr(self.rcps_tab, '_rcps_table_data')
                    and self.rcps_tab._rcps_table_data is not None):
                ...
                for _, r in table.iterrows():
                    ...
                    if not aid or aid in ('RA', 'RS', ''):
                        continue
                    es = r.get('actual_start', r.get('early_start', 0))
                    dur = r.get('duration', 0)
                    ef_vals.append(es + dur)
                if ef_vals:
                    rcps_dur = f"{max(ef_vals):.0f}"
        except Exception:
            pass
    ...
    def get_rcps_table_data(self):
        ...
        try:
            # Method 1: Through app's _rcps_tab (edu app uses underscore
            # prefix)
            rcps_tab = getattr(
                self.app,
                '_rcps_tab',
            ...
            # Method 3: Fallback through tab reference set by
            # set_rcps_tab_reference()
    ...
    def create_analyzer_from_rcps_table(self, rcps_table):
        ...
        try:
            ...
            base_analyzer = getattr(
                self.app, "current_analyzer", None) or getattr(
                self.app, "base_analyzer", None)
            original_activities = {}
            print(f"[DEBUG] Base analyzer found: {base_analyzer}")
            print(f"[DEBUG] Base analyzer type: {type(base_analyzer)}")
            print(
                f"[DEBUG] Base analyzer has activities: {
                    hasattr(
                        base_analyzer,
                        'activities') if base_analyzer else 'No analyzer'}")

            if base_analyzer and hasattr(base_analyzer, 'activities'):
                print(
                    f"[DEBUG] Base analyzer activities count: {len(base_analyzer.activities)}")
                for activity in base_analyzer.activities:
                    original_activities[activity['id']] = activity
                    print(
                        f"[DEBUG] Original activity {
                            activity['id']}: crash_cost={
                            activity.get(
                                'crash_cost',
                                0)}, normal_cost={
                            activity.get(
                ...
                print(
                    "[DEBUG] No base analyzer or no activities found. Using default cost values.")
            ...
            for _, row in activity_rows.iterrows():
                ...
                min_duration = original_activity.get('min_duration', max(
                    1, int(duration * 0.6)) if duration > 0 else duration)

                # 🔍 DEBUG: Track cost data retrieval for each activity
                if activity_id not in ['RA', 'RS']:
                    print(f"   [COST DEBUG] Activity {activity_id}:")
                    print(
                        f"      Original activity found: {
                            activity_id in original_activities}")
                    if activity_id in original_activities:
                        orig_act = original_activities[activity_id]
                        print(
                            f"      Original crash_cost: {
                                orig_act.get(
                                    'crash_cost',
                                    'MISSING')}")
                        print(
                            f"      Original normal_cost: {
                                orig_act.get(
                                    'normal_cost',
                                    'MISSING')}")
                ...
                activity = {
                    ...
                    'duration': duration,
                    # CRITICAL: Use actual_start as early_start for
                    # ProjectCrashing
                    'early_start': actual_start,
                    ...
                    # FIXED: Use original crash/normal costs from base analyzer
                    # (not RCPS table)
                ...
                print(
                    f"[DEBUG] Activity {
                        ...
                        activity['min_duration']}, crash_cost={
                        activity['crash_cost']}, normal_cost={
            ...
            for activity in activities:
                ...
                # Create node with proper timing attributes AND crashing
                # attributes
                node_attrs = {
                ...
                print(
                    f"[DEBUG] Node {
                        activity['id']}: duration={duration}, min_duration={
                        node_attrs['min_duration']}, crash_cost={
                        node_attrs['crash_cost']}")
                ...
                if predecessors:
                    ...
                    for pred in pred_list:
                        if pred in G.nodes():
                            ...
                            print(
                                f"[DEBUG] Added edge: {pred} -> {activity['id']}")
            ...
            print(
                f"[RCPS TABLE] Created analyzer with {
                    len(activities)} activities")
            print(f"[RCPS TABLE] EF values in graph: {ef_values}")
            print(
                f"[RCPS TABLE] Calculated project duration: {
                    analyzer.project_duration}")

            # CRITICAL: Also set the duration attribute that ProjectCrashing
            # might use
            ...
            for node_id in G.nodes():
                ...
                print(
                    f"  Node {node_id}: ES={
                        node_data.get('ES')}, EF={
                        node_data.get('EF')}, Duration={
                        node_data.get('duration')}")
    ...
    def run_crashing(self):
        ...
        try:
            ...
            try:
                target_duration = int(
                    round(float(self.target_duration_var.get())))
            except ValueError:
                messagebox.showerror(
                    "Input Error", "Target Duration must be an integer.")
                return
            ...
            try:
                ...
                messagebox.showerror(
                    "Input Error", "Invalid strategy selected.")
                return
            ...
            try:
                ...
                messagebox.showerror(
                    "Input Error", "Invalid objective selected.")
                return

            max_budget = self.budget_var.get()
            try:
                max_budget = int(round(float(max_budget))
                                 ) if max_budget else None
            except ValueError:
                messagebox.showerror(
                    "Input Error", "Max Budget must be an integer or blank.")
                return

            max_crash_cost = self.max_crash_cost_var.get()
            try:
                max_crash_cost = int(
                    round(float(max_crash_cost))) if max_crash_cost else None
            except ValueError:
                messagebox.showerror(
                    "Input Error",
            ...
            print(
                f"[RCPS TABLE] ✅ Got RCPS table data with shape: {
                    rcps_table_data.shape}")
            ...
            rcps_analyzer = self.create_analyzer_from_rcps_table(
                rcps_table_data)
            if rcps_analyzer is None:
                messagebox.showerror(
                    "Data Error", "Failed to create analyzer from RCPS data.")
                return

            print(
                f"[RCPS TABLE] ✅ Created analyzer from RCPS data: {
                    type(rcps_analyzer)}")
            ...
            if hasattr(rcps_analyzer, 'G') and rcps_analyzer.G is not None:
                try:
                    ...
                    ef_values = [
                        rcps_analyzer.G.nodes[node].get(
                            'EF', 0) for node in rcps_analyzer.G.nodes()]
                    ...
                    print(
                        f"[RCPS TABLE] ✅ RCPS project duration: {rcps_project_duration}")
                    ...
                    messagebox.showwarning(
                        "Duration Check", f"Could not validate target duration: {e}")
            else:
                print(
                    f"[ERROR] Analyzer validation failed - G attribute: {hasattr(rcps_analyzer, 'G')}")
            ...
            print(
                f"[DEBUG] Before ProjectCrashing - analyzer.project_duration: {
                    getattr(
                        rcps_analyzer,
                        'project_duration',
                        'NOT SET')}")
            print(
                f"[DEBUG] Before ProjectCrashing - analyzer.duration: {
                    getattr(
                        rcps_analyzer,
                        'duration',
                        'NOT SET')}")
            print(
                f"[DEBUG] Before ProjectCrashing - analyzer attributes: {
                    [
                        attr for attr in dir(rcps_analyzer) if 'duration' in attr.lower()]}")
            ...
            if hasattr(rcps_analyzer, 'G') and rcps_analyzer.G:
                ef_values_before = [
                    rcps_analyzer.G.nodes[node].get(
                        'EF', 0) for node in rcps_analyzer.G.nodes()]
                print(
                    f"[DEBUG] EF values in analyzer.G before ProjectCrashing: {ef_values_before}")
                print(
                    f"[DEBUG] Max EF in analyzer.G: {
                        max(ef_values_before) if ef_values_before else 0}")
                ...
                print("=" * 70)
                print(
                    f"📊 Analyzer graph nodes: {len(rcps_analyzer.G.nodes())}")
            ...
            class RCPSProjectCrashing(ProjectCrashing):
                ...
                def _lowest_cost_strategy(
                    ...
                    print(
                        f"[RCPS STRATEGY] Starting with target: {target_duration}")
                    ...
                    # RCPS-AWARE: Use existing EF values (don't recalculate
                    # with NetworkBuilder)
                    ...
                    print(
                        f"[RCPS STRATEGY] RCPS original_duration: {original_duration}")

                    # Early termination check
                    if current_duration <= target_duration:
                        print(
                            f"[RCPS STRATEGY] Target already achieved: {current_duration} <= {target_duration}")
                    ...
                    print(
                        f"[RCPS STRATEGY] Starting crashing iterations from {current_duration} to {target_duration}")
                    ...
                    while current_duration > target_duration and iterations < max_iterations:
                        iterations += 1
                        print(
                            f"[RCPS STRATEGY] === Iteration {iterations} ===")
                        print(
                            f"[RCPS STRATEGY] Current: {current_duration} -> Target: {target_duration}")
                        ...
                        print(
                            f"[RCPS STRATEGY] Critical activities: {critical_activities}")

                        if not critical_activities:
                            print(
                                "[RCPS STRATEGY] No critical activities found")
                        ...
                        for activity in critical_activities:
                            ...
                            min_dur = G.nodes[activity].get(
                                'min_duration', current_dur)
                            crash_cost = G.nodes[activity].get(
                                'crash_cost', 100)
                            ...
                            if current_dur > min_dur and crash_cost < best_crash_cost:
                                ...
                                print(
                                    f"[RCPS STRATEGY] {activity} can crash: {current_dur}→{min_dur}, cost={crash_cost}")

                        if best_activity is None:
                            print(
                                "[RCPS STRATEGY] No activities can be crashed further")
                            termination_reason = "No activities can be crashed further"
                            break

                        # BUDGET CHECKS: Check if applying this crash would
                        # exceed any budget limits
                        ...
                        for node in G.nodes:
                            if node not in ['START', 'END']:
                                node_normal_cost = G.nodes[node].get(
                                    'normal_cost', 50)
                                step_normal_cost += node_normal_cost

                        # Check max_budget limit
                        if max_budget is not None:
                            projected_total_cost = total_normal_cost + \
                                step_normal_cost + total_crash_cost + potential_crash_cost
                            if projected_total_cost > max_budget:
                                print(
                                    f"[RCPS STRATEGY] Budget limit would be exceeded in the next step. Projected: {projected_total_cost}, Max: {max_budget}")
                        ...
                        if max_crash_cost is not None:
                            ...
                            if projected_crash_cost > max_crash_cost:
                                print(
                                    f"[RCPS STRATEGY] Crash cost limit would be exceeded in the next step. Projected: {projected_crash_cost}, Max: {max_crash_cost}")
                                termination_reason = "Crash cost limit would be exceeded in the next step"
                                break
                        ...
                        if max_normal_cost is not None:
                            ...
                            if projected_normal_cost > max_normal_cost:
                                print(
                                    f"[RCPS STRATEGY] Normal cost limit would be exceeded in the next step. Projected: {projected_normal_cost}, Max: {max_normal_cost}")
                                termination_reason = "Normal cost limit would be exceeded in the next step"
                                break
                        ...
                        # Recalculate EF for this activity (preserve RCPS
                        # logic)
                        ...
                        print(
                            f"[RCPS STRATEGY] Crashed {best_activity}: {current_dur} -> {new_duration}, EF: {new_ef}")

                        # CRITICAL: Recalculate project duration and propagate changes
                        # Since we changed a critical activity, we need to
                        ...
                        print(
                            f"[RCPS STRATEGY] Project duration after crash: {current_duration} -> {new_project_duration}")

                        # If no improvement, something's wrong
                        if new_project_duration >= current_duration:
                            print(
                                f"[RCPS STRATEGY] WARNING: No duration improvement after crashing {best_activity}")
                        ...
                        # Create proper crash log entry matching expected
                        # format for report generation
                        crash_amount = current_dur - new_duration
                        critical_path = [
                            node for node in G.nodes if G.nodes[node].get(
                                'float', 0) == 0 and node not in [
                                'START', 'END']]
                        normal_cost = G.nodes[best_activity].get(
                            'normal_cost', 50)

                        # Get list of active activities
                        active_activities = [
                            node for node in G.nodes if node not in [
                                'START', 'END']]
                        ...
                        print(
                            f"[RCPS STRATEGY] Added crash log entry: iteration={iterations}, activity={best_activity}, cost={best_crash_cost}, duration={new_duration}")

                        print(
                            f"[RCPS STRATEGY] Iteration {iterations} completed: Project duration now {current_duration}")

                        # Check if target reached
                        if current_duration <= target_duration:
                            print(
                                f"[RCPS STRATEGY] Target {target_duration} reached!")
                    ...
                    print(
                        f"[RCPS STRATEGY] Completed - Original: {original_duration}, Final: {current_duration}, Iterations: {iterations}")

                    # Debug: Print complete crash log before returning result
                    print(
                        f"[RCPS STRATEGY] Final crash log has {
                            len(crash_log)} entries:")
                    for i, entry in enumerate(crash_log):
                        print(
                            f"  Entry {
                                i +
                                1}: activity={
                                entry.get('activity')}, cost={
                                entry.get('cost')}, duration={
                                entry.get('duration')}")
                ...
                def _update_downstream_timing(self, G, changed_activity):
                    ...
                    print(
                        f"[RCPS STRATEGY] Updating downstream timing for {changed_activity}")

                    # Simple approach: find activities that have this as predecessor
                    # and update their ES/EF if needed
                    for node in G.nodes:
                        if node in [
                                'START', 'END'] or node == changed_activity:
                            continue
                        ...
                        if changed_activity in predecessors:
                            ...
                            if pred_ef != current_es:  # Only update if different
                                ...
                                print(
                                    f"[RCPS STRATEGY] Updated {node}: ES={pred_ef}, EF={
                                        pred_ef + duration}")

            crashing_engine = RCPSProjectCrashing(
                rcps_analyzer)  # Now uses RCPS-aware engine!

            print("[RCPS TABLE] Using RCPS-aware ProjectCrashing engine")
            ...
            print(
                f"[DEBUG] After crashing run - result initial duration: {
                    getattr(
                        result,
                        'original_duration',
                        'NOT SET')}")
            print(
                f"[DEBUG] After crashing run - result final duration: {
                    getattr(
                        result,
                        'final_duration',
                        'NOT SET')}")
            ...
            print(
                "[RCPS TABLE] RCPS crashing with table data completed successfully!")
            ...
            messagebox.showerror(
                "Error",
                f"RCPS Crashing with table data failed: {
                    str(e)}")
            import traceback
            traceback.print_exc()
    ...
    def _add_rcps_info_to_summary(self, result, resource_limit):
        ...
        try:
            ...
            rcps_info = ""
            rcps_info += "🔍 RCPS Processing Details:\\n"
            ...
            resource_efficiency = (time_saved * resource_limit) / \
                result.total_crash_cost if result.total_crash_cost > 0 else 0

            rcps_info += "📈 Resource-Aware Performance Metrics:\\n"
            rcps_info += f"   • Time Reduction: {time_saved} days\\n"
            rcps_info += f"   • Resource Efficiency: {
                resource_efficiency:.3f} (time·resource)/cost\\n"
            rcps_info += f"   • Cost per Day Saved: ${
                result.total_crash_cost /
                time_saved:.2f}\\n" if time_saved > 0 else "   • Cost per Day Saved: N/A (no time saved)\\n"

            # Add comparison note
            rcps_info += "\\n⚠️  IMPORTANT NOTE:\\n"
            rcps_info += "   This analysis uses resource-constrained starting times,\\n"
            rcps_info += "   NOT theoretical CPM times. Results reflect real project\\n"
            rcps_info += "   constraints and are more realistic than CPM-only crashing.\\n"
    ...
    def _enhance_rcps_metrics(self, result, resource_limit):
        ...
        try:
            ...
            rcps_metrics = f"\\n{'=' * 50}\\n"
            rcps_metrics += "RCPS-SPECIFIC METRICS\\n"
            rcps_metrics += f"{'=' * 50}\\n"

            # Resource utilization metrics
            time_saved = result.original_duration - result.final_duration
            total_resources_available = resource_limit * result.final_duration
            cost_per_resource_day = result.total_crash_cost / \
                total_resources_available if total_resources_available > 0 else 0

            rcps_metrics += "Resource Utilization Analysis:\\n"
            rcps_metrics += f"   • Resource Limit: {resource_limit} units\\n"
            rcps_metrics += f"   • Project Duration (Final): {
                result.final_duration} days\\n"
            rcps_metrics += f"   • Total Resource-Days Available: {total_resources_available}\\n"
            rcps_metrics += f"   • Crash Cost per Resource-Day: ${
                cost_per_resource_day:.4f}\\n\\n"

            # RCPS vs CPM comparison note
            rcps_metrics += "RCPS Analysis Benefits:\\n"
            rcps_metrics += "   • Uses actual project delays from resource constraints\\n"
            rcps_metrics += "   • Starting duration reflects real resource limitations\\n"
            rcps_metrics += "   • Crashing decisions consider resource availability\\n"
            rcps_metrics += "   • More accurate than theoretical CPM-only analysis\\n\\n"

            # Efficiency comparison
            if time_saved > 0:
                resource_time_efficiency = (
                    time_saved * resource_limit) / result.total_crash_cost
                rcps_metrics += "Resource-Time Efficiency:\\n"
                rcps_metrics += "   • (Time Saved × Resource Limit) / Crash Cost\\n"
                rcps_metrics += f"   • ({time_saved} × {resource_limit}) / ${
                    result.total_crash_cost:.2f}\\n"
                rcps_metrics += f"   • = {
                    resource_time_efficiency:.4f} resource-days per dollar\\n"
    ...
    def _create_basic_results_display(self, result):
        ...
        if result is None:
            for text_widget in [
                ...
                if hasattr(self, text_widget.winfo_name()):
                    ...
                    text_widget.insert(
                        '1.0', "No RCPS crashing results to display.")
            return

        # Basic summary
        summary = "RCPS Crashing Analysis Results\\n"
        summary += f"{'=' * 40}\\n"
        summary += f"Original Duration: {result.original_duration} days\\n"
        summary += f"Final Duration: {result.final_duration} days\\n"
        summary += f"Time Saved: {result.original_duration -
                                  result.final_duration} days\\n"
        ...
        # Basic metrics
        metrics_content = "Basic RCPS Metrics:\\n"
        metrics_content += f"Total Cost: ${
            result.total_crash_cost +
            result.total_normal_cost:.2f}\\n"
        metrics_content += f"Computation Time: {
            result.computation_time:.3f} seconds\\n"
    ...
    def export_results(self):
        ...
        messagebox.showinfo(
            "Export Results",
            "RCPS Crashing export functionality would run here.")
    ...
    def open_results_in_new_window(self):
        ...
        if not summary_content or summary_content == "No results to display.":
            messagebox.showwarning(
                "No Results",
                "No RCPS crashing results to display. Please run RCPS crashing analysis first.")
            return
    ...
    def save_results_from_window(
        ...
        try:
            ...
            save_dir = filedialog.askdirectory(
                title="Choose directory to save RCPS results")
            if not save_dir:
                return
            ...
            summary_file = os.path.join(
                save_dir, f"rcps_crashing_summary_{timestamp}.txt")
            with open(summary_file, 'w', encoding='utf-8') as f:
                f.write(summary_content)

            # Save log
            log_file = os.path.join(
                save_dir, f"rcps_crashing_log_{timestamp}.txt")
            with open(log_file, 'w', encoding='utf-8') as f:
                f.write(log_content)

            # Save metrics
            metrics_file = os.path.join(
                save_dir, f"rcps_crashing_metrics_{timestamp}.txt")
