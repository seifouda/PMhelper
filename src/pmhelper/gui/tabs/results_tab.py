...
class ResultsTab:
    ...
    def create_summary_frame(self):
        ...
        summary_frame = ttk.LabelFrame(self.paned_window, text="Project Summary", padding="10")
        ...
        # Left column labels
        self.project_duration_label = ttk.Label(left_frame, text="Project Duration: --", 
                                               font=("Arial", 10, "bold"))
        self.project_duration_label.pack(anchor="w", pady=2)
        
        self.critical_path_label = ttk.Label(left_frame, text="Critical Path: --")
        self.critical_path_label.pack(anchor="w", pady=2)
        
        self.total_activities_label = ttk.Label(left_frame, text="Total Activities: --")
        self.total_activities_label.pack(anchor="w", pady=2)
        
        self.critical_activities_label = ttk.Label(left_frame, text="Critical Activities: --")
        ...
        self.expected_duration_label = ttk.Label(self.probability_frame, text="Expected Duration: --")
        self.expected_duration_label.pack(anchor="w", pady=2)
        
        self.variance_label = ttk.Label(self.probability_frame, text="Project Variance: --")
        self.variance_label.pack(anchor="w", pady=2)
        
        self.std_deviation_label = ttk.Label(self.probability_frame, text="Standard Deviation: --")
    ...
    def create_activities_frame(self):
        ...
        activities_frame = ttk.LabelFrame(self.paned_window, text="Activity Details", padding="5")
    ...
    def create_critical_path_frame(self):
        ...
        cp_frame = ttk.LabelFrame(self.paned_window, text="Critical Path", padding="10")
    ...
    def setup_cmp_activities_tree(self):
        ...
        # CPM columns
        columns = ("ID", "Activity", "Duration", "ES", "EF", "LS", "LF", "Float", "Critical")
    ...
    def update_summary(self):
        ...
        self.project_duration_label.config(text=f"Project Duration: {project_duration}")
        
        critical_path = self.results_data.get('critical_path', [])
        cp_text = " → ".join(critical_path) if critical_path else "--"
        self.critical_path_label.config(text=f"Critical Path: {cp_text}")
        
        activities = self.results_data.get('activities', [])
        total_activities = len(activities)
        critical_activities = len([a for a in activities if a.get('critical', False)])
        
        self.total_activities_label.config(text=f"Total Activities: {total_activities}")
        self.critical_activities_label.config(text=f"Critical Activities: {critical_activities}")
        
        # PERT-specific information
        if self.analysis_mode == 'probabilistic':
            expected_duration = self.results_data.get('expected_duration', '--')
    ...
    def clear_results(self):
        ...
        self.critical_path_label.config(text="Critical Path: --")
        ...
        for activity in activities:
            if self.analysis_mode == 'probabilistic':
                values = (
                    ...
                    activity.get('most_likely', ''),
                    activity.get('pessimistic', ''),
                    str(activity.get('expected', activity.get('duration', 0))),  # Show integer expected duration for scheduling
                    f"{activity.get('variance', 0):.3f}",
                    activity.get('ES', ''),  
                    activity.get('EF', ''),  
                    activity.get('LS', ''), 
    ...
    def update_critical_path(self):
        ...
        if critical_path:
            ...
            for activity in critical_activities:
                ...
                # FIXED: Show float value for each critical activity (should be 0)
                float_val = activity.get('float', 0)
                path_text += f"• {activity.get('id', '')} - {activity.get('name', '')} (Duration: {duration}, Float: {float_val})\n"
        ...
        with open(filename, 'w', newline='', encoding='utf-8') as file:
            if filename.lower().endswith('.csv'):
                ...
                for activity in activities:
                    if self.analysis_mode == 'probabilistic':
                        row = [
                            ...
                            f"{activity.get('float', 0):.2f}",  # FIXED: Use 'float'
                            "Yes" if activity.get('critical', False) else "No"
                        ]
                    else:
                        row = [
                ...
                for activity in activities:
                    ...
                    file.write(f"  Earliest Start: {activity.get('ES', '')}\n")  # FIXED
                    file.write(f"  Earliest Finish: {activity.get('EF', '')}\n")  # FIXED
                    file.write(f"  Latest Start: {activity.get('LS', '')}\n")  # FIXED
                    file.write(f"  Latest Finish: {activity.get('LF', '')}\n")  # FIXED
                    file.write(f"  Total Float: {activity.get('float', 0):.2f}\n")  # FIXED
                    file.write(f"  Critical: {'Yes' if activity.get('critical', False) else 'No'}\n")
