...
class RCPSTab:
    ...
    def display_hybrid_schedule_view(self, parent, cpm_table, rcps_table, df_gantt):
        ...
        # Configure grid weights with stronger proportions
        main_frame.grid_rowconfigure(0, weight=1)
        main_frame.grid_rowconfigure(1, weight=1)
        main_frame.grid_columnconfigure(0, weight=1, minsize=100)  # Tables column (1/5 width) - minimum 100px
        main_frame.grid_columnconfigure(1, weight=4)
        # Diagnostics: print actual frame sizes after rendering
        def print_frame_sizes():
            main_frame.update_idletasks()
            table_width = main_frame.grid_bbox(0, 0)[2]
            chart_width = main_frame.grid_bbox(1, 0)[2]
            print(f"[DIAG] Table column width: {table_width}px, Chart column width: {chart_width}px")
        main_frame.after(500, print_frame_sizes)
        
        # Create compact tables (without timeline columns to save space)
        cpm_compact = self.create_compact_table(cpm_table)
        rcps_compact = self.create_compact_table(rcps_table)
        
        # Left side: Compact tables
        self.display_compact_schedule_table(main_frame, cpm_compact, "Initial CPM-based Plan", col=0, row=0)
        self.display_compact_schedule_table(main_frame, rcps_compact, "Resource-Constrained Schedule (RCPS)", col=0, row=1)
    ...
    parent.grid_columnconfigure(col, weight=1)
        ...
        def adjust_compact_column_widths(event=None):
            ...
            if num_cols == 0:
                return
            min_col_width = 1  # Minimum possible width
            max_col_width = 70
            available_width = max(frame_width - 20, num_cols * min_col_width)
            ...
            sheet.set_column_widths(widths)
            print(f"[DIAG] Compact table frame width: {frame_width}px, columns: {num_cols}, col_width: {col_width}")
