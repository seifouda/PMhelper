...
class CharterForm(ttk.Frame):
    ...
    def __init__(self, parent, charter: Charter, template: Template, on_change: Optional[Callable] = None):
        """
    ...
    def _create_section(self, section: TemplateSection):
        ...
        for field_def in section.fields:
            self._create_field(content, section.section_id, field_def, row, section_widget)
            row += 1
    
    def _create_field(self, parent: ttk.Frame, section_id: str, field_def: TemplateField, row: int, section_widget=None):
        ...
        wrapper_frame = tk.Frame(parent, bg='SystemButtonFace', highlightthickness=0)
        wrapper_frame.grid(row=row, column=1, sticky="ew", padx=(0, 10), pady=5)
        ...
        if field_def.field_type == "text":
            ...
            widget = self._create_text_field(wrapper_frame, field_def)  # Default to text
        ...
        if field_def.help_text:
            ...
            help_label.grid(row=row, column=2, sticky="w", padx=(5, 10), pady=5)
    ...
    def _create_text_field(self, parent, field_def: TemplateField) -> ttk.Entry:
        ...
        entry.bind('<KeyRelease>', lambda e: self._on_field_change(entry, field_def))
        entry.bind('<FocusOut>', lambda e: self._on_field_change(entry, field_def))
        
        return entry
    
    def _create_textarea_field(self, parent, field_def: TemplateField) -> scrolledtext.ScrolledText:
        """Create a textarea field."""
        rows = field_def.rows or 4
        textarea = scrolledtext.ScrolledText(parent, height=rows, width=50, wrap=tk.WORD)
        
        # Track if we're programmatically setting text to avoid triggering change events
        ...
        if field_def.placeholder:
            ...
            def on_focus_in(e):
                if textarea.get('1.0', tk.END).strip() == field_def.placeholder:
                    textarea._loading = True
        ...
        def on_text_change(event):
            ...
            if not getattr(textarea, '_loading', False) and textarea.edit_modified():
                self._on_field_change(textarea, field_def)
                textarea.edit_modified(False)  # Reset the modified flag
        
        textarea.bind('<<Modified>>', on_text_change)
        textarea.bind('<FocusOut>', lambda e: self._on_field_change(textarea, field_def))
        
        return textarea
    
    def _create_date_field(self, parent, field_def: TemplateField) -> DatePicker:
        """Create a date picker field."""
        date_picker = DatePicker(parent, callback=lambda v: self._on_field_change(date_picker, field_def))
        return date_picker
    
    def _create_currency_field(self, parent, field_def: TemplateField) -> ttk.Entry:
        ...
        def on_focus_out(e):
            ...
            # Remove any existing currency symbols (longest first to handle multi-char symbols)
            value = var.get().strip()
            sorted_symbols = sorted(self.CURRENCY_SYMBOLS.values(), key=len, reverse=True)
    ...
    def _update_currency_fields(self):
        ...
        for field_id, entry in self.currency_fields.items():
            ...
            # Remove old currency symbols - sort by length (longest first) to handle multi-char symbols
            value = current_value
            sorted_symbols = sorted(self.CURRENCY_SYMBOLS.values(), key=len, reverse=True)
            for symbol in sorted_symbols:
                if symbol:
    ...
    def _create_number_field(self, parent, field_def: TemplateField) -> ttk.Entry:
    ...
    def _create_dropdown_field(self, parent, field_def: TemplateField) -> ttk.Combobox:
    ...
    def _create_list_field(self, parent, field_def: TemplateField) -> ListEditor:
    ...
    def _create_table_field(self, parent, field_def: TemplateField) -> TableEditor:
        """Create a table editor field."""
        columns = field_def.columns or []
        table = TableEditor(parent, columns, callback=lambda data: self._on_field_change(table, field_def))
        return table
    ...
    def _get_widget_value(self, widget, field_type: str) -> Any:
        ...
        if field_type == "textarea":
            ...
            # Strip all currency symbols before saving (longest first to handle multi-char symbols)
    ...
    def _load_data(self):
        ...
        for field_key, info in self.field_widgets.items():
            ...
            value = self.charter.get_field_value(section_id, field_def.field_id)
    ...
    def validate(self) -> bool:
        ...
        for field_key, info in self.field_widgets.items():
            ...
            # Validate required
            if field_def.required:
                valid, error = FieldValidator.validate_required(value, field_def.field_label)
                if not valid:
            ...
            if value:
                if field_def.field_type == "date":
                    valid, error = FieldValidator.validate_date(value, field_def.field_label)
                    if not valid:
                        self.validation_errors[field_key] = error
                        if wrapper:
                            show_validation_error(wrapper, error)
                    ...
                    valid, error = FieldValidator.validate_currency(value, field_def.field_label)
                    if not valid:
                        self.validation_errors[field_key] = error
                        if wrapper:
                            show_validation_error(wrapper, error)
                    ...
                    valid, error = FieldValidator.validate_number(value, field_def.field_label)
                    if not valid:
                        self.validation_errors[field_key] = error
                        if wrapper:
                            show_validation_error(wrapper, error)
                ...
                if field_def.max_length and field_def.field_type in ["text", "textarea"]:
                    valid, error = FieldValidator.validate_max_length(value, field_def.max_length, field_def.field_label)
                    if not valid:
                        self.validation_errors[field_key] = error
                        if wrapper:
                            show_validation_error(wrapper, error)
        ...
        # If there are errors, expand the section with first error and scroll to it
    ...
    def _do_scroll(self, widget):
        ...
        try:
            ...
            if frame_height > canvas_height:
                ...
                scroll_pos = max(0.0, min(1.0, scroll_pos))  # Clamp between 0 and 1
            ...
            def flash_step(count):
                if count > 0:
                    ...
                    if isinstance(widget, (ttk.Entry, ttk.Combobox)):
                        ...
                        widget.configure(background='#ffcccc' if count % 2 == 1 else current_bg)
