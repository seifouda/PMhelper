...
class InputTab:
    ...
    def create_button_frame(self):
        ...
        ttk.Button(
            button_frame,
            text="Load Sample CPM",
            command=self.load_sample_cpm).pack(
            side=tk.LEFT,
            padx=(
                0,
                10))
        ttk.Button(
            button_frame,
            text="Load Sample PERT",
            command=self.load_sample_pert).pack(
            side=tk.LEFT,
            padx=(
                0,
                10))
        ...
        ttk.Button(
            button_frame,
            text="? Help",
            command=self.main_window.show_input_tab_help).pack(
            side=tk.RIGHT,
            padx=(
                10,
                0))
        ...
        ttk.Button(
            ...
            command=self.main_window.analyze_project).pack(
            side=tk.RIGHT)
    ...
    def validate_file_format(self, filename, expected_mode):
        ...
        try:
            ...
            # If we can't read the file, let the regular loading process handle
            # the error
            return True
    ...
    def load_probabilistic_data(self):
        ...
        if filename:
            ...
            if self.validate_file_format(filename, 'probabilistic'):
                ...
                messagebox.showerror(
                    "Invalid Data Format", "This file appears to contain CPM data (with duration column only).\n\n"
    ...
    def load_file(self, filename, mode):
        ...
        try:
            ...
            self.main_window.set_status(
                f"Loaded {len(activities_data)} activities from {filename}")
    ...
    def clear_all(self):
        ...
        result = messagebox.askyesno(
            "Clear All", "Are you sure you want to clear all data?")
        if result:
            try:
                ...
                print(
                    f"Warning: Tried to clear tree but it was not valid. Error: {e}")
    ...
    def edit_item(self, event):
        ...
        if bbox:
            ...
            self.edit_entry.bind(
                '<Return>', lambda e: self.save_edit(
                    item, col_index))
            self.edit_entry.bind('<Escape>', lambda e: self.cancel_edit())
            self.edit_entry.bind(
                '<FocusOut>', lambda e: self.save_edit(
                    item, col_index))
        ...
        if filename:
            try:
                with open(filename, 'r', newline='', encoding='utf-8') as file:
                    ...
                    for row in reader:
                        if detected_mode == 'probabilistic':
                            ...
                            activity_dict = {
                                'id': row.get(
                                    'id', ''), 'activity': row.get(
                                    'activity', ''), 'duration': row.get(
                                    'duration', ''), 'predecessors': row.get(
                                    'predecessors', ''), 'min_duration': row.get(
                                    'min_duration', ''), 'crash_cost': row.get(
                                    'crash_cost', ''), 'resource_demand': row.get(
                                    'resource_demand', ''), 'normal_cost': row.get(
                                    'normal_cost', '')}
                        activities_data.append(activity_dict)
                    self.populate_tree(activities_data)
                    ...
                    self.main_window.set_status(
                        f"Loaded {
                            len(activities_data)} activities with auto-detected {detected_mode} mode")
            except Exception as e:
                messagebox.showerror(
