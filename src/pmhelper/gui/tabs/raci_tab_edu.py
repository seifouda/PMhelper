...
from pmhelper.core.raci_model import RACIMatrix, CELL_COLORS, VALID_VALUES
...
class _RACISubTab:
    ...
    def get_matrix(self) -> RACIMatrix:
        ...
        rows = [v.get().strip() for v in self._row_name_vars if v.get().strip()]
        cols = [v.get().strip() for v in self._col_name_vars if v.get().strip()]
    ...
    def _build_ui(self):
        ...
        ttk.Button(toolbar, text="🔍 Validate",
                   command=self._run_validation).pack(side=tk.LEFT, padx=(0, 4))
        ...
        ttk.Label(legend, text="Legend:", font=("TkDefaultFont", 9, "bold")).pack(
            side=tk.LEFT, padx=(0, 6))
        ...
        # Click-to-cycle hint
        ttk.Label(legend, text="← click cell to assign",
                  foreground="#6b7280", font=("TkDefaultFont", 8, "italic")).pack(
    ...
    def _rebuild_grid(self, row_names: List[str], col_names: List[str],
        ...
        tk.Label(self._grid_inner, text="", width=18, relief="groove",
                 bg="#e5e7eb").grid(row=0, column=0, padx=1, pady=1, sticky="nsew")
        ...
        for r_idx, rname in enumerate(row_names):
            ...
            for c_idx in range(n_cols):
                ...
                if preserve_values:
                    try:
                        old_r = old_row_names.index(rname) if rname in old_row_names else -1
                        old_c = old_col_names.index(col_names[c_idx]) if col_names[c_idx] in old_col_names else -1
                ...
                for widget in (cell_f, cell_lbl):
                    widget.bind("<Button-1>",
                                lambda e, r=r_idx, c=c_idx: self._cycle_cell(r, c))

                # Watch for external var changes (e.g. load)
                cell_var.trace_add("write", lambda *a, r=r_idx, c=c_idx: self._refresh_cell_color(r, c))
    ...
    def _remove_col(self, col_idx: int):
        ...
        cols = [v.get().strip() for v in self._col_name_vars]
        if len(cols) <= 1:
            messagebox.showinfo("Remove Column", "Cannot remove the last column.")
            return
        current_matrix = self.get_matrix()
        cols.pop(col_idx)
        new_matrix = RACIMatrix(rows=rows, cols=cols)
    ...
    def _run_validation(self):
        ...
        if not m.rows:
            self._val_text.insert(tk.END, "Add activities and roles to validate.\n", "warning")
        ...
        if not issues:
            ...
            if errors:
                self._val_text.insert(tk.END, f"✗ {len(errors)} error(s):\n", "header")
                for iss in errors:
                    self._val_text.insert(tk.END, f"• {iss.message}\n", "error")
            if warnings:
                self._val_text.insert(tk.END, f"\n⚠ {len(warnings)} warning(s):\n", "header")
                for iss in warnings:
                    self._val_text.insert(tk.END, f"• {iss.message}\n", "warning")
