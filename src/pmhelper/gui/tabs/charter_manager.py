...
class CharterManager(ttk.Frame):
    ...
    def _create_charter_list(self):
        ...
        self.tree.heading(
            'title',
            text='Project Title',
            command=lambda: self._sort_by('title'))
        self.tree.heading(
            'status',
            text='Status',
        ...
        self.tree.heading(
            'completion',
            text='Completion',
            command=lambda: self._sort_by('completion'))
    ...
    def _update_charter_list_with_sort(self, sort_key: str):
        ...
        if sort_key == 'modified_desc':
            filtered_charters.sort(
                key=lambda c: c.get(
                    'modified_date', ''), reverse=True)
        ...
        if selection:
            ...
            if tags:
                ...
                self.selected_charter = next(
                    (c for c in self.charters if c.get('filepath') == filepath), None)
    ...
    def _on_duplicate_charter(self):
        ...
        try:
            ...
            if new_filepath:
                self._update_status(
                    f"Charter duplicated: {
                        Path(new_filepath).name}")
                self._load_charters()
