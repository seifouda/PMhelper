...
class CPMAnalyzer:
    ...
    def crash_project_with_rcps(self, target_duration, resource_limit, priority_rule='minimum_slack', max_iterations=300, max_budget=None):
        ...
        while iteration < max_iterations:
            ...
            print(f"Critical activities (zero float): {' -> '.join(critical_activities)}")
    ...
    def crash_project(self, target_duration, max_iterations=300, max_budget=None):
        ...
        while current_duration > target_duration and iteration < max_iterations:
            ...
            # Find crashable activities on critical path that haven't finished yet
            ...
            for activity in critical_activities:
                ...
                if can_crash:
                    ...
                    print(f"  Crashable: {activity} (dur={current_dur}, min={min_dur}, cost={crash_cost}, ES={es}, EF={ef})")
                elif not not_finished:
                    print(f"  Not crashable: {activity} (already finished, EF={ef} <= current_time={current_time})")
                else:
                    print(f"  Not crashable: {activity} (dur={current_dur}, min={min_dur}, already at min or max steps reached)")
    ...
    def debug_crash_process(self, crashed_G, crash_log, target_duration):
        ...
        # Initial state
        initial_duration = max([original_G.nodes[node]['EF'] for node in original_G.nodes()])
        ...
        for i, entry in enumerate(crash_log):
            ...
            new_project_duration = max([current_G.nodes[node]['EF'] for node in current_G.nodes()])
            ...
            debug_text += f"  Completed Activities: {sorted(list(completed_activities))}\n"
            ...
            if in_progress:
                ...
                for act in in_progress:
                    debug_text += f"      - {act['id']}: Cost={act['crash_cost']}, "
                    debug_text += f"Duration={act['duration']}, Min={act['min_duration']}\n"
        ...
        debug_text += f"  Total Reduction: {initial_duration - final_duration}\n"
        debug_text += f"  Total Crash Cost: {sum(entry['crash_cost'] for entry in crash_log)}\n"
        ...
        if final_duration <= target_duration:
            ...
            debug_text += f"  Reason: {self._get_termination_reason(crashed_G, crash_log, target_duration)}\n"
    ...
    def _get_termination_reason(self, crashed_G, crash_log, target_duration):
        ...
        for act in critical_activities:
            if (crashed_G.nodes[act]['duration'] > crashed_G.nodes[act]['min_duration'] and
        ...
        if crash_log:
            ...
            if 'project_duration_reduction' in last_entry and last_entry['project_duration_reduction'] == 0:
    ...
    def debug_rcps_crash_process(self, crashed_G, crash_log, target_duration, resource_limit, priority_rule):
        """
        ...
        initial_schedule = self.generate_rcps_schedule_for_graph(original_G, resource_limit, priority_rule)
        ...
        for i, entry in enumerate(crash_log):
            ...
            current_schedule = self.generate_rcps_schedule_for_graph(current_G, resource_limit, priority_rule)
            current_duration = current_schedule['project_duration']
            ...
            # Debug output for this step
            debug_text += f"Iteration {step_num}: Critical path = {' -> '.join(critical_activities)}\n"
            ...
            for activity in critical_activities:
                if activity not in ['START', 'END']:
                    ...
                    if activity in completed_activities:
                        ...
                        debug_text += f"EF={current_schedule['activities'][activity]['actual_finish']} <= current_time={current_time})\n"
                        continue
            ...
            # Get original duration from entry or from the graph before the current iteration
            orig_duration = entry.get('original_duration', current_G.nodes[activity_id]['duration'] + 1)
            debug_text += f"  Crashing {activity_id}: {orig_duration} -> {new_duration} (Cost: {crash_cost})\n"
            
            # Get resulting duration after crashing
            resulting_duration = entry.get('resulting_project_duration', current_duration)
        ...
        debug_text += f"Crash optimization complete. Final duration: {final_duration}, Total cost: {sum(entry['crash_cost'] for entry in crash_log)}\n"
    ...
    def _get_rcps_termination_reason(self, crashed_G, crash_log, target_duration, resource_limit, priority_rule):
        ...
        resource_usage = self._calculate_resource_usage(final_schedule, crashed_G)
...
class CPMDesktopApp:
    ...
    def create_input_tab(self):
        ...
        self.add_row_btn = ttk.Button(button_frame, text="Add Row", command=self.add_row)
        self.add_row_btn.pack(side=tk.LEFT, padx=(0, 10))
        
        self.delete_row_btn = ttk.Button(button_frame, text="Delete Row", command=self.delete_row)
        ...
        ttk.Button(button_frame, text="Download Sample Deterministic CSV", command=self.download_sample_deterministic_csv).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(button_frame, text="Download Sample Probabilistic CSV", command=self.download_sample_probabilistic_csv).pack(side=tk.LEFT, padx=(0, 10))
        
        # Analysis button
        ttk.Button(button_frame, text="Analyze", command=self.analyze_project).pack(side=tk.RIGHT)
        
        # Mode indicator label
        self.mode_label = ttk.Label(input_frame, text="Mode: None", font=("Arial", 10, "bold"))
    ...
    def create_probability_analysis_tab(self):
        ...
        ttk.Label(title_frame, text="PERT Probability Analysis", font=("Arial", 14, "bold")).pack(side=tk.LEFT)
        self.prob_status_label = ttk.Label(title_frame, text="Status: Disabled (Load probabilistic data first)", 
                                          font=("Arial", 10), foreground="red")
        self.prob_status_label.pack(side=tk.RIGHT)
        
        # Project statistics frame
        stats_frame = ttk.LabelFrame(scrollable_frame, text="Project Statistics", padding=10)
        stats_frame.pack(fill=tk.X, padx=10, pady=5)
        
        self.stats_text = tk.Text(stats_frame, height=8, state=tk.DISABLED, wrap=tk.WORD)
        self.stats_text.pack(fill=tk.BOTH, expand=True)
        
        # Analysis controls frame
        controls_frame = ttk.LabelFrame(scrollable_frame, text="Probability Calculations", padding=10)
        ...
        ttk.Label(comp_frame, text="1. Project Completion Probability:", font=("Arial", 11, "bold")).pack(anchor="w")
        ...
        self.calc_prob_btn = ttk.Button(comp_input_frame, text="Calculate Probability", 
                                       command=self.calculate_completion_probability, state=tk.DISABLED)
        self.calc_prob_btn.pack(side=tk.LEFT, padx=5)
        
        self.prob_result_label = ttk.Label(comp_input_frame, text="", font=("Arial", 10, "bold"))
        ...
        ttk.Label(dur_frame, text="2. Required Duration for Given Probability:", font=("Arial", 11, "bold")).pack(anchor="w")
        
        dur_input_frame = ttk.Frame(dur_frame)
        dur_input_frame.pack(fill=tk.X, pady=5)
        
        ttk.Label(dur_input_frame, text="Target Probability (0-1):").pack(side=tk.LEFT)
        self.target_probability_var = tk.StringVar()
        self.target_probability_entry = ttk.Entry(dur_input_frame, textvariable=self.target_probability_var, width=10)
        self.target_probability_entry.pack(side=tk.LEFT, padx=(5, 10))
        
        self.calc_duration_btn = ttk.Button(dur_input_frame, text="Calculate Duration", 
                                           command=self.calculate_required_duration, state=tk.DISABLED)
        self.calc_duration_btn.pack(side=tk.LEFT, padx=5)
        
        self.duration_result_label = ttk.Label(dur_input_frame, text="", font=("Arial", 10, "bold"))
        self.duration_result_label.pack(side=tk.LEFT, padx=(10, 0))
        
        # Visualization frame
        viz_frame = ttk.LabelFrame(scrollable_frame, text="Normal Distribution Visualization", padding=10)
        viz_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)
        
        self.fig_probability = plt.Figure(figsize=(12, 6))
        self.canvas_probability = FigureCanvasTkAgg(self.fig_probability, viz_frame)
    ...
    def download_sample_deterministic_csv(self):
        ...
        if file_path:
            try:
                ...
                messagebox.showinfo("Success", f"Sample deterministic CSV saved to:\n{file_path}")
    ...
    def analyze_project(self):
        ...
        try:
            if not self.analysis_mode:
                messagebox.showwarning("Warning", "Please load data first (deterministic or probabilistic).")
                return
            ...
            if self.analysis_mode == 'deterministic':
                G, critical_paths, critical_activities = self.cpm_analyzer.analyze(activities_data)
                analysis_type = "CPM"
            else:  # probabilistic
                G, critical_paths, critical_activities = self.pert_analyzer.analyze(activities_data)
                analysis_type = "PERT"
            ...
            messagebox.showinfo("Success", f"{analysis_type} analysis completed successfully!")
    ...
    def save_results_csv(self):
        ...
        try:
            ...
            if not self.current_analyzer or not hasattr(self.current_analyzer, 'G') or self.current_analyzer.G is None:
    ...
    def display_results(self, G, critical_paths, analysis_type="CPM"):
        ...
        if analysis_type == "PERT":
            ...
            for node in G.nodes():
                if node not in ['START', 'END']:  # Skip START and END nodes
                    table_data.append([
                        node,
                        f"{G.nodes[node]['expected_time']:.2f}",  # Expected time (original)
            ...
            headers = ["Activity", "Expected Time", "Duration (Ceil)", "Variance", "ES", "EF", "LS", "LF", "Float", "Critical"]
        ...
        table_data.sort(key=lambda x: int(x[4] if analysis_type == "PERT" else x[2]))
        ...
        if analysis_type == "PERT" and hasattr(self, 'pert_analyzer'):
            results_text += f"PROJECT VARIANCE: {self.pert_analyzer.project_variance}\n"
            results_text += f"PROJECT STANDARD DEVIATION: {self.pert_analyzer.project_std}\n"
    ...
    def run_rcps_tables(self):
        ...
        try:
            ...
            df_gantt = self.generate_gantt_chart(self.current_analyzer.G, self.current_analyzer.critical_activities)
    ...
    def _validate_resource_limit(self, resource_limit):
        ...
        for node in self.current_analyzer.G.nodes():
            if node not in ['START', 'END']:
                demand = self.current_analyzer.G.nodes[node].get('resource_demand', 0)
                try:
                    demand = int(demand)
                except Exception:
                    demand = 0
    ...
    def _calculate_and_display_rcps_tables(self, df_gantt, resource_limit):
        ...
        cpm_table, time_cols, critical_ids = self.current_analyzer.build_cpm_schedule_table(df_gantt, resource_limit)
        rcps_table, actual_starts, critical_ids2 = self.current_analyzer.rcps_heuristic_schedule_table(df_gantt, resource_limit, priority_rule)
        
        # Pad CPM table columns if RCPS has more time periods
        cpm_table_updated, time_cols_updated = self._pad_cpm_table_columns(cpm_table, rcps_table, time_cols)
        
        # Plot the comparison tables
        self.plot_schedule_tables(cpm_table_updated, rcps_table, time_cols_updated, critical_ids, actual_starts, df_gantt)
    ...
    def _pad_cpm_table_columns(self, cmp_table, rcps_table, time_cols):
        ...
        """
        cmp_time_cols = [col for col in cmp_table.columns if isinstance(col, int)]
        ...
        if max_time_rcps > max_time_cmp:
            ...
            non_time_cols = [col for col in cmp_table.columns if not isinstance(col, int)]
    ...
    def _prepare_rcps_data_for_crashing(self, df_gantt, resource_limit):
        ...
        # Create activities list in the format expected by crashing optimization
        activities_list = []
        ...
        for idx, row in rcps_table.iterrows():
            ...
            if activity_id in self.current_analyzer.G.nodes:
                ...
                print(f"WARNING: Activity {activity_id} not found in analyzer graph")
                continue
            ...
            try:
                ...
                print(f"WARNING: Invalid duration for activity {activity_id}, using original")
                duration = original_data.get('duration', 0)
            
            # Get activity name
            activity_name = original_data.get('activity', '')
            
            # Get predecessors from original graph (maintain dependency structure)
            predecessors = []
            if activity_id in self.current_analyzer.G.nodes:
            ...
            try:
                actual_start = int(actual_start) if not pd.isna(actual_start) else 0
            except (ValueError, TypeError):
                actual_start = 0
    ...
    def update_cpm_graph_with_rcps(self):
        ...
        for idx, row in rcps_df.iterrows():
            ...
            if act_id in updated_G.nodes and act_id not in ['RA', 'RS']:
                try:
                    ...
                    if 'actual_start' in row and not pd.isna(row['actual_start']):
                    ...
                    if 'activity' in row and row['activity'] and not pd.isna(row['activity']):
                        updated_G.nodes[act_id]['activity'] = row['activity']
                        fields_transferred['activity'] += 1
                        
                    if 'min_duration' in row and not pd.isna(row['min_duration']):
                        updated_G.nodes[act_id]['min_duration'] = int(row['min_duration'])
                        fields_transferred['min_duration'] += 1
                        
                    if 'crash_cost' in row and not pd.isna(row['crash_cost']):
                        updated_G.nodes[act_id]['crash_cost'] = float(row['crash_cost'])
                        fields_transferred['crash_cost'] += 1
                        
                    if 'resource_demand' in row and not pd.isna(row['resource_demand']):
                        updated_G.nodes[act_id]['resource_demand'] = int(row['resource_demand'])
        ...
        project_duration = max([updated_G.nodes[node]['EF'] for node in updated_G.nodes()])
        ...
        for node in reversed(list(nx.topological_sort(updated_G))):
            if node not in ['START', 'END']:
                ...
                if successors:
                    min_succ_ls = min([updated_G.nodes[succ]['LS'] for succ in successors])
                    updated_G.nodes[node]['LF'] = min_succ_ls
                else:
                    updated_G.nodes[node]['LF'] = project_duration
                
                updated_G.nodes[node]['LS'] = updated_G.nodes[node]['LF'] - duration
                updated_G.nodes[node]['float'] = updated_G.nodes[node]['LS'] - updated_G.nodes[node]['ES']
        
        print(f"Created temporary graph with RCPS data for crashing (original graph preserved)")
        return updated_G
    ...
    def create_crashing_tab(self):
        ...
        self.crash_results_text = scrolledtext.ScrolledText(results_frame, font=("Courier", 10), height=8)
        ...
        # Step display area (this expands and fills the rest)
        self.step_display_frame = ttk.Frame(results_frame, width=1610, height=500)
        self.step_display_frame.pack_propagate(False)  # Prevent resizing to fit children
        ...
        step_nav_frame = ttk.Frame(crashing_frame, borderwidth=2, relief="solid", height=40)
        step_nav_frame.pack_propagate(False)  # Prevent shrinking to fit children
        step_nav_frame.pack(fill=tk.X, side=tk.BOTTOM)
        
        # Navigation controls
        ttk.Button(step_nav_frame, text="◀◀ First", command=self.show_first_step).pack(side=tk.LEFT, padx=2)
        ttk.Button(step_nav_frame, text="◀ Previous", command=self.show_previous_step).pack(side=tk.LEFT, padx=2)
        ...
        ttk.Button(step_nav_frame, text="Next ▶", command=self.show_next_step).pack(side=tk.LEFT, padx=2)
        ttk.Button(step_nav_frame, text="Last ▶▶", command=self.show_last_step).pack(side=tk.LEFT, padx=2)

        # Step selection
        ttk.Label(step_nav_frame, text="Go to step:").pack(side=tk.LEFT, padx=(20, 5))
        self.step_select_var = tk.StringVar(value="0")
        self.step_select_spinbox = ttk.Spinbox(step_nav_frame, from_=0, to=0, width=5,
    ...
    def run_crashing_optimization(self, max_budget=None):
        ...
        try:
            ...
            for i, tab_id in enumerate(self.notebook.tabs()):
                if self.notebook.tab(tab_id, "text") == "Crashing Optimization":
            ...
            current_duration = max([self.current_analyzer.G.nodes[node]['EF'] for node in self.current_analyzer.G.nodes()])
            ...
            crashed_G, total_crash_cost, crash_log = self.current_analyzer.crash_project(target_duration, max_budget=max_budget)
    ...
    def display_crash_results(self, crashed_G, total_crash_cost, crash_log, target_duration):
        ...
        initial_duration = max([self.current_analyzer.G.nodes[node]['EF'] for node in self.current_analyzer.G.nodes()])
        ...
        if new_duration <= target_duration:
            ...
            lines.append(f"✗ Target duration not achieved (stopped at {new_duration})")
    ...
    def show_crashing_steps_diagrams(self, crashed_G, crash_log):
        ...
        for i, entry in enumerate(crash_log):
            ...
            self.step_graphs.append((i+1, entry['activity'], entry['new_duration'], G.copy()))
    ...
    def show_step(self, step_index):
        ...
        if 0 <= step_index < len(self.step_graphs):
            ...
            if step_num == 0:
                ...
                ax.set_title(f"Step {step_num}: Activity {activity} crashed to {new_duration}", 
    ...
    def create_rcps_crashing_tab(self):
        ...
        # Text output for crash log/results
        self.rcps_crash_results_text = scrolledtext.ScrolledText(results_frame, font=("Courier", 10), height=8)
        ...
        # Navigation controls
        ttk.Button(rcps_step_nav_frame, text="◀◀ First", command=self.show_rcps_first_step).pack(side=tk.LEFT, padx=2)
        ttk.Button(rcps_step_nav_frame, text="◀ Previous", command=self.show_rcps_previous_step).pack(side=tk.LEFT, padx=2)
        
        self.rcps_step_info_label = ttk.Label(rcps_step_nav_frame, text="Step 0 of 0")
        self.rcps_step_info_label.pack(side=tk.LEFT, padx=10)
        
        ttk.Button(rcps_step_nav_frame, text="Next ▶", command=self.show_rcps_next_step).pack(side=tk.LEFT, padx=2)
        ttk.Button(rcps_step_nav_frame, text="Last ▶▶", command=self.show_rcps_last_step).pack(side=tk.LEFT, padx=2)
        
        # Step selection
        ttk.Label(rcps_step_nav_frame, text="Go to step:").pack(side=tk.LEFT, padx=(20, 5))
        self.rcps_step_select_var = tk.StringVar(value="0")
        self.rcps_step_select_spinbox = ttk.Spinbox(rcps_step_nav_frame, from_=0, to=0, width=5, 
    ...
    def run_rcps_crashing_optimization(self):
        ...
        try:
            ...
            if not hasattr(self, 'last_rcps_table') or self.last_rcps_table is None:
                messagebox.showwarning("Warning", "Please run the RCPS scheduling first in the RCPS tab.")
                return
                
            # Check if CPM analysis has been performed
            if not hasattr(self.current_analyzer, 'G') or self.current_analyzer.G is None:
            ...
            try:
                ...
                messagebox.showerror("Error", "Please set a valid resource limit in the RCPS tab first.")
                return
            ...
            if rcps_updated_graph is None:
                messagebox.showerror("Error", "Failed to create RCPS-updated graph.")
                return
            
            # Get RCPS duration after update
            rcps_duration = max([rcps_updated_graph.nodes[node]['EF'] for node in rcps_updated_graph.nodes()])
            ...
            try:
                ...
                self.show_rcps_crashing_steps_diagrams(rcps_updated_graph, crash_log)  # Use RCPS base graph
    ...
    def display_rcps_crashing_results(self, crashed_G, total_crash_cost, crash_log, target_duration):
        ...
        # CRITICAL FIX: Get initial duration from RCPS (first crash log entry or current graph)
        if crash_log and 'resulting_project_duration' in crash_log[0]:
            # Get initial duration from first crash log entry
            # The initial duration should be the "before" state of the first crash
            first_entry = crash_log[0]
            if 'project_duration_reduction' in first_entry and 'resulting_project_duration' in first_entry:
                initial_duration = first_entry['resulting_project_duration'] + first_entry['project_duration_reduction']
            else:
        ...
        try:
            ...
            final_schedule = self.current_analyzer.generate_rcps_schedule_for_graph(crashed_G, resource_limit, priority_rule)
            rcps_critical_activities = final_schedule['critical_activities']
            
            if rcps_critical_activities:
                lines.append(f"Final Critical Path: {' -> '.join(rcps_critical_activities)}")
            else:
    ...
    def show_rcps_crashing_steps_diagrams(self, base_G, crash_log):
        ...
        for i, entry in enumerate(crash_log):
            ...
            G = self.current_analyzer.backward_pass(G)
            G = self.current_analyzer.calculate_float(G)
            self.rcps_step_graphs.append((i+1, entry['activity'], entry['new_duration'], G))
        
        self.rcps_total_steps = len(self.rcps_step_graphs)
        
        print(f"DEBUG: RCPS Total step graphs created: {self.rcps_total_steps}")
    ...
    def show_rcps_step(self, step_index):
        ...
        if 0 <= step_index < len(self.rcps_step_graphs):
            ...
            # Set title
            if step_num == 0:
                ax.set_title("Initial RCPS Network", fontsize=14, fontweight='bold')
            else:
                ax.set_title(f"RCPS Step {step_num}: Activity {activity} crashed to {new_duration}", 
                            fontsize=14, fontweight='bold')
            ...
            # Update step info
            self.rcps_step_info_label.config(text=f"Step {step_index} of {self.rcps_total_steps-1}")
    ...
    def show_rcps_all_steps_grid(self):
        ...
        for idx, (step_num, activity, new_duration, G_step) in enumerate(self.rcps_step_graphs):
            fig = plt.Figure(figsize=(4, 3))
            ax = fig.add_subplot(111)
            ...
            if step_num == 0:
                ax.set_title("Initial RCPS Network", fontsize=10, fontweight='bold')
            else:
                ax.set_title(f"RCPS Step {step_num}: {activity} → {new_duration}", 
                            fontsize=10, fontweight='bold')
    ...
    def enable_probability_analysis(self):
        ...
        self.prob_status_label.config(text="Status: Enabled (Probabilistic data loaded)", foreground="green")
    ...
    def disable_probability_analysis(self):
        ...
        self.probability_tab_enabled = False
        self.prob_status_label.config(text="Status: Disabled (Load probabilistic data first)", foreground="red")
    ...
    def update_probability_analysis(self):
        ...
        stats_text = f"Project Expected Duration: {stats['expected_duration']:.1f} time units\n"
        stats_text += f"Project Variance: {stats['variance']:.3f}\n"
        stats_text += f"Project Standard Deviation: {stats['std_deviation']:.3f}\n\n"
        ...
        for activity_info in stats['critical_activities_variance']:
            stats_text += f"{activity_info['id']}: Variance = {activity_info['variance']:.3f}, "
    ...
    def calculate_completion_probability(self):
        ...
        try:
            ...
            self.plot_probability_distribution(target_duration=target_duration, highlight_prob=probability)
    ...
    def calculate_required_duration(self):
        ...
        try:
            ...
            duration = self.pert_analyzer.calculate_duration_for_probability(target_probability)
            
            result_text = f"Required Duration: {duration:.2f} time units"
            self.duration_result_label.config(text=result_text, foreground="blue")
            
            # Update plot with this calculation
            self.plot_probability_distribution(target_probability=target_probability, highlight_duration=duration)
            
        except ValueError as e:
            messagebox.showerror("Error", f"Invalid probability value: {str(e)}")
        except Exception as e:
            messagebox.showerror("Error", f"Calculation failed: {str(e)}")
    
    def plot_probability_distribution(self, target_duration=None, highlight_prob=None, 
                                    target_probability=None, highlight_duration=None):
        ...
        try:
            ...
            if target_duration is not None and highlight_prob is not None:
                ...
                ax.fill_between(x_shade, y_shade, alpha=0.6, color='green', 
                               label=f'P(T ≤ {target_duration}) = {highlight_prob:.4f}')
            ...
            if target_probability is not None and highlight_duration is not None:
                ...
                ax.axvline(highlight_duration, color='orange', linestyle='-', linewidth=2,
                          label=f'Duration for P = {target_probability:.3f}: {highlight_duration:.2f}')
            ...
            ax.set_title('Project Completion Time Distribution (Normal)', fontsize=12, fontweight='bold')
            ax.legend(fontsize=9)
            ax.grid(True, alpha=0.3)
            
            # Add statistics text
            stats_text = f'μ = {expected_duration:.2f}, σ = {std_dev:.3f}, σ² = {stats["variance"]:.3f}'
